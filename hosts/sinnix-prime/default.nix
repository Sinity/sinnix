{ inputs, lib, ... }:
{
  ...
  sinnix.services = {
    ...
    # Phase 3a (sinnix-9pd): per-window screen frames, promoted to
    # default-on via this host enable per the mkServiceModule factory
    # contract (services are default-off at the module level; hosts
    # express the opt-in) rather than baking a non-standard default into
    # the factory itself.
