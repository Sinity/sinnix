# Hyprland window rules configuration
{
  windowrule = [
    ...
    "float 1, pin 1, size 480 270, move 100%-500 50, match:title ^(Picture-in-Picture)$"
    ...
    "float 1, size 40% 45%, move 2% 55%, opacity 0.8 0.8, match:class ^(blueman-manager)$"
    "opacity 0.8 0.8, match:class ^(pwvucontrol)$"

    "float 1, center 1, size 85% 85%, workspace special:scratch_term silent, match:class ^(scratchpad-terminal)$"
    "float 1, center 1, size 80% 80%, workspace special:scratch_notes silent, match:class ^(notes-scratch)$"
    "float 1, center 1, size 70% 40%, workspace special:scratch_rawlog silent, match:class ^(rawlog-capture)$"
    "float 1, center 1, size 85% 85%, workspace special:scratch_spotify silent, match:class ^([Ss]potify)$"
    ...
    "size 60% 100%, match:title ^(session: )"
    "move 0% 0%, match:title ^(session: )"
    "tile 1, match:class ^(qutebrowser)$"
    "group set, match:class ^(qutebrowser)$"
    "float 1, size 28% 24%, move 70% 6%, match:class ^(qutebrowser)$, match:float true"
