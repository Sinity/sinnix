...
let
  ...
  # btrfs snapshots do NOT recurse into nested subvolumes: a snapshot of
  # /realm renders each one as an EMPTY DIRECTORY. btrbk declared only
  # `subvolume .`, so every /realm/state/* database was represented in Borg by
  # a bare directory entry and nothing else -- verified 2026-08-16 against
  # archive realm-realm.20260816T173000+0200, which held exactly one entry for
  # state/polylogue and one for state/sinex.
  #
  # That was ~230G of live service state with no backup, and it explains the
  # estate's accumulated workarounds: /realm/staging exists to copy data OUT
  # of these subvolumes into a plain directory the snapshot stream can see,
  # and the dozen hand-made polylogue backups scattered through /realm/inbox
  # and /realm/data were the only thing putting that store on the backup drive
  # at all.
  #
  # Each entry gets a btrbk subvolume line, its own snapshot family (btrbk
  # names them <basename>.TIMESTAMP in the same snapshot dir) and its own
  # drain into the realm repository, so dedup still spans the whole of /realm.
  # sinex/state/blob-repository is deliberately absent: it is a CAS with its
  # own always-live job (borgbackup-job-sinex-blobs).
  stateSubvolumes = [
    "polylogue"
    "sinex"
    "machine-telemetry"
  ...
  # sinex/state/blob-repository is a 168G CAS already archived daily by
  # borgbackup-job-sinex-blobs into its OWN repository. Borg dedup is
  # per-repository, so letting the state-sinex drain take it too would store a
  # second full copy rather than a cheap reference.
  stateExtraExcludes = {
    sinex = [ "state/blob-repository" ];
  };
  ...
  backupNestedSubvolumeCoverage = {
    "swap" = "regenerable swapfile, deliberately not backup material";
    "home/sinity/.cache" =
  ...
  # One drain per nested subvolume: mkSnapshotDrainScript resolves a single
  # snapshot family (latest match wins; older ones are deleted once covered),
  # so the families cannot share a service.
  stateDrainConfig = lib.mkMerge (
    map (name: {
      sinnix.runtime.surfaces."borgbackup-job-state-${name}" = {
        unit = "borgbackup-job-state-${name}.service";
      ...
      systemd.services."borgbackup-job-state-${name}" = {
        description = "Drain /realm/state/${name} btrbk snapshots into Borg";
        ...
        serviceConfig = (backupServiceConfig "borgbackup-job-state-${name}.service") // {
          Type = "oneshot";
          TimeoutStopSec = "15s";
        };
        path = with pkgs; [
        ...
        script = mkSnapshotDrainScript {
          ...
          snapshotGlob = "${name}.*";
          bindTarget = "${borgSnapshotBindRoot}/state-${name}";
          archivePrefix = "state-${name}";
          minIntervalSec = borgDrainMinIntervalSec;
          exclude = realmExcludes ++ (stateExtraExcludes.${name} or [ ]);
        };
      };

      # Offset from the realm drain's :15,:35,:55 so the two do not contend for
      # the global Borg lock on every wake.
      systemd.timers."borgbackup-job-state-${name}" = {
