...
let
  ...
  mcpContext7Bin = pkgs.writeShellScriptBin "mcp-context7" ''
    set -euo pipefail
    exec npx -y @context7/server
  '';

  mcpFirecrawlBin = pkgs.writeShellScriptBin "mcp-firecrawl" ''
    set -euo pipefail
    exec npx -y mcp-server-firecrawl
  '';
...
in
{
  options.sinnix.features.dev.utilities = {
    enable = lib.mkEnableOption "Programming utilities and tools";
  };
    ...
    home-manager.users.${user} =
      ...
      in
      {
        ...
        home.file = {
          ...
          # Disabled due to pure eval issue
          # ".local/bin/ai".source = config.lib.file.mkOutOfStoreSymlink (
          #   sinnix.paths.projectRoot + "/scripts/ai"
          # );
