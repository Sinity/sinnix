...
let
  ...
  machineTelemetry = pkgs.writeTextFile {
    ...
    text = ''
      ...
      def run_cmd(args: list[str], timeout: float = 3.0) -> subprocess.CompletedProcess[str] | None:
          try:
      ...
      def gpu_metrics() -> tuple[dict[str, object], list[str]]:
          ...
          safe("gpu_power_limit_w", lambda: pynvml.nvmlDeviceGetEnforcedPowerLimit(h) / 1000.0)
          safe("gpu_temp_c", lambda: float(pynvml.nvmlDeviceGetTemperature(h, pynvml.NVML_TEMPERATURE_GPU)))
          ...
          safe("gpu_clock_mhz", lambda: float(pynvml.nvmlDeviceGetClockInfo(h, pynvml.NVML_CLOCK_GRAPHICS)))
          safe("gpu_mem_clock_mhz", lambda: float(pynvml.nvmlDeviceGetClockInfo(h, pynvml.NVML_CLOCK_MEM)))
          try:
      ...
      def hardware_state(host: str, rapl_zones: list[dict[str, object]]) -> dict[str, object]:
          ...
          return {
              ...
              "cpu_no_turbo": int_or_none(read_text("/sys/devices/system/cpu/intel_pstate/no_turbo")),
      ...
      def init_db(conn: sqlite3.Connection) -> None:
          ...
          ensure_columns(conn, "metric_sample", {
              "cpu_psi_some_avg60": "REAL",
              "cpu_psi_some_avg300": "REAL",
              "cpu_psi_some_total_us": "REAL",
              "io_psi_some_avg60": "REAL",
      ...
      def parse_ping(ip: str) -> dict[str, object]:
          ...
          if proc is None:
              return {"ip": ip, "loss": 100, "min_ms": None, "avg_ms": None, "max_ms": None, "status": "timeout"}
      ...
      def network_probe(host: str, boot_id: str | None, interface: str, gateway: str, *, do_bloat: bool) -> dict[str, object]:
          gaps: list[str] = []
          observed_at = now_iso()
          ping = {
          ...
          if do_bloat:
              ...
              try:
                  curl_proc = subprocess.Popen(
                      ["curl", "-4", "-o", "/dev/null", "-s", "--max-time", "6", "https://speed.cloudflare.com/__down?bytes=10000000"],
          ...
          iface = {}
          for key in ("rx_bytes", "tx_bytes", "rx_errors", "tx_errors", "rx_dropped", "tx_dropped", "collisions"):
          ...
          pmtu = run_cmd(["ping", "-c", "1", "-W", "2", "-M", "do", "-s", "1464", "8.8.8.8"], timeout=4)
          conntrack = {
      ...
      def memory_metrics() -> dict[str, int | None]:
          ...
          return {
              "mem_used_mb": int((total - avail) / 1024) if total is not None and avail is not None else None,
              "mem_avail_mb": int(avail / 1024) if avail is not None else None,
              "swap_used_mb": int((swap_total - swap_free) / 1024) if swap_total is not None and swap_free is not None else None,
          }

      def systemctl_props(unit: str, *, user: bool = False, user_name: str | None = None) -> dict[str, str]:
          cmd = ["systemctl"]
          if user:
              if user_name:
          ...
          cmd += [
              ...
              "-p", "ControlGroup", "-p", "MemoryCurrent",
      ...
      def insert_service_states(conn: sqlite3.Connection, host: str, boot_id: str | None, units: list[str], user_name: str) -> None:
          observed_at = now_iso()
          rows = []
          for unit in units:
              ...
              if not props:
                  continue
              rows.append((
                  observed_at, host, boot_id, unit, "user" if user else "system",
                  props.get("ActiveState"), props.get("SubState"), int_or_none(props.get("MainPID")),
                  props.get("ControlGroup"), int_or_none(props.get("MemoryCurrent")),
                  int_or_none(props.get("CPUUsageNSec")), int_or_none(props.get("IOReadBytes")),
      ...
      def main() -> int:
          ...
          parser.add_argument("--user-name", default="${username}")
          ...
          with sqlite3.connect(db) as conn:
              ...
              if args.network_probe_once:
                  ...
                  conn.execute(
                      ...
                      ("machine.network", now_iso(), "ok", None, json.dumps({"inserted": inserted})),
                  )
                  conn.commit()
                  return 0
              ...
              conn.execute(
                  ...
                  (now_iso(), args.host, boot_id, SCHEMA_VERSION, json.dumps(state, sort_keys=True)),
              )
              conn.execute(
                  "INSERT OR REPLACE INTO source_status (source, checked_at, status, reason, payload_json) VALUES (?, ?, ?, ?, ?)",
                  ("machine-telemetry", now_iso(), "ok", None, json.dumps({"schema_version": SCHEMA_VERSION})),
              )
              conn.commit()
              ...
              while True:
                  ...
                  if not any(Path("/sys/class/hwmon").glob("hwmon*/fan*_input")):
                  ...
                  row = {
                      ...
                      "cpu_pkg_c": sysfs_temp_c(coretemp / "temp1_input") if coretemp else None,
                  ...
                  if args.network_interval > 0 and sample_start >= next_network:
                      do_bloat = args.bufferbloat_interval > 0 and sample_start - last_bloat >= args.bufferbloat_interval
