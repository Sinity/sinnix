...
mkServiceModule {
  ...
  configFn =
    { cfg, config, ... }:
    lib.mkMerge [
      # Two units, neither of which fits mkServiceModule's single-unit `job`
      # sugar (this module owns both), so both go through direct
      # lib.sinnix.mkScheduledJob calls with their own hand-registered
      # surfaces threaded in for failure-notify coverage.
      (lib.sinnix.mkScheduledJob
        {
          inherit config;
          unitName = "sinnix-sandbox-audit";
          description = "Check no unit is confined out of its declared output";
          surface = config.sinnix.runtime.surfaces.sandbox-audit-check;
        }
        {
      ...
      # kernelAudit gates the WHOLE mkScheduledJob call result via mkIf, not
      # a job-spec-shape conditional: forcing this same-module cfg option to
      # decide the job spec's own attribute-key presence (e.g. via
      # `lib.optionalAttrs cfg.kernelAudit { ... }` inside the job spec)
      # would force it during the module system's generic config-merge pass,
      # which is a genuine self-referential blackhole for a same-module
      # option (see modules/lib/scheduled-job.nix's `timer.enable` doc
      # comment, and sinnix-oracle's job function for the same failure mode
      # confirmed empirically). mkIf is the recognized marker the merge pass
      # defers instead of forcing eagerly.
      (lib.mkIf cfg.kernelAudit (
        lib.sinnix.mkScheduledJob
          {
            inherit config;
            unitName = "sinnix-audit-drain";
            description = "Drain kernel audit denial records into the capture lake";
            surface = config.sinnix.runtime.surfaces.sandbox-audit-drain;
          }
          {
