          ...
          // {
            ...
            ExecStartPre = [
              ...
              "${pkgs.bash}/bin/bash -c ${lib.escapeShellArg ''
                ...
                if [ ! -r "$manifest" ]; then
                  echo "clodex: Claude Code is not patched; run clodex patch" >&2
                  exit 78
                fi
                binary="$(${pkgs.jq}/bin/jq -er '.binaryPath' "$manifest")"
                expected="$(${pkgs.jq}/bin/jq -er '.claudeVersion' "$manifest")"
                if [ ! -x "$binary" ]; then
                  echo "clodex: patched Claude Code binary is missing: $binary; run clodex patch" >&2
                  exit 78
                fi
                ...
                if [ -z "$actual" ] || [ "$actual" != "$expected" ] || [ "$size" != "$patched_size" ]; then
                  echo "clodex: Claude Code patch is stale (expected $expected/$patched_size, found ''${actual:-unknown}/$size); run clodex patch" >&2
                  exit 78
                fi
              ''}"
            ];
