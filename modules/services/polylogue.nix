...
let
  ...
  tmpRules = map (dir: "d ${dir} 0755 root root - -") [ chatlogRoot inboxRoot archiveRoot configRoot stateRoot ];
  ...
  services = {
    "polylogue-watch-gemini" = mkService "gemini" [ "sync" "drive" "--watch" "--out" (archiveRoot + "/gemini") ];
    "polylogue-watch-codex" = mkService "codex" [ "sync" "codex" "--watch" "--out" (archiveRoot + "/codex") ];
    "polylogue-watch-claude-code" = mkService "claude-code" [ "sync" "claude-code" "--watch" "--out" (archiveRoot + "/claude-code") ];
    "polylogue-watch-chatgpt" = mkService "chatgpt" [ "sync" "chatgpt" "--watch" "--base-dir" inboxRoot "--out" (archiveRoot + "/chatgpt") ];
    "polylogue-watch-claude" = mkService "claude" [ "sync" "claude" "--watch" "--base-dir" inboxRoot "--out" (archiveRoot + "/claude") ];
  };
  writeConfig = ''
...
in {
  ...
  config = mkIf config.services.polylogue-watch.enable {
