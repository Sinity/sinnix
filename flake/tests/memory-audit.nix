  ...
  perSystem =
    ...
    in
    {
      checks.memory-audit = pkgs.runCommand "sinnix-memory-audit-check" {
        ...
        printf '%s\n' '# 2020-01-01 /missing/path' 'This is obsolete and needs review.' > "$fixture/in-scope/MEMORY.md"
        printf '%s\n' '# 2020-01-01 /missing/ignored' > "$fixture/out-of-scope/MEMORY.md"
        ...
        jq -e 'select(.kind == "dead_path" and .line == 1 and .value == "/missing/path")' "$TMPDIR/report.jsonl" >/dev/null
        jq -e 'select(.kind == "review_required" and .line == 2)' "$TMPDIR/report.jsonl" >/dev/null
        ! jq -e 'select(.value == "/missing/ignored")' "$TMPDIR/report.jsonl" >/dev/null
        touch "$out"
      '';
    };
}
