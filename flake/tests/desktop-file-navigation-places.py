...
lines = [line for line in pathlib.Path(bookmarks_file).read_text().splitlines() if line.strip()]
...
failures = []
for line, declared_path in zip(lines, declared):
    ...
    if run.returncode != 0 or "standard::type: 2" not in run.stdout:
        failures.append(f"{uri}: GIO did not resolve it to a directory: {run.stderr.strip()}")
    else:
