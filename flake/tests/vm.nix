      ...
      vmChecks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        sinnixd-vm = mkVmCheck system {
          ...
          testScript = ''
            ...
            machine.succeed(f"{as_user} agentctl status | jq -e '.ok and .payload.value.queue.available and (.payload.value.queue.missing_pools | length == 0)' >/dev/null")
            machine.wait_until_succeeds(f"{as_user} pueue status --json | jq -e --arg job {job_id} '[.tasks[] | select(.label | endswith($job))] | length == 1' >/dev/null")
            # pueued's own slice is the ceiling for every task it runs.
            machine.succeed(f"grep -q '/sinnixd-work.slice/' /proc/{parent}/cgroup")
            ...
            machine.wait_until_succeeds(pressure_clear)
            # The pytest group runs one task at a time, so a second submission
            # waits in the queue instead of competing for the host.
            first_serial_id = machine.succeed(f"{as_user} agentctl job start sinnix serial_stress | jq -r '.payload.value.job_id'").strip()
            machine.wait_until_succeeds(f"{as_user} agentctl job get {first_serial_id} | jq -e '.payload.value.state.phase == \"running\"' >/dev/null")
            second_serial_id = machine.succeed(f"{as_user} agentctl job start sinnix serial_stress | jq -r '.payload.value.job_id'").strip()
            machine.succeed(f"{as_user} agentctl job get {second_serial_id} | jq -e '.payload.value.state.phase == \"queued\"' >/dev/null")
            machine.succeed(f"{as_user} agentctl job wait {first_serial_id} --timeout-seconds 30 | jq -e '.ok and .payload.value.state.phase == \"succeeded\"' >/dev/null")
            machine.succeed(f"{as_user} agentctl job wait {second_serial_id} --timeout-seconds 30 | jq -e '.ok and .payload.value.state.phase == \"succeeded\"' >/dev/null")
          '';
        };
