...
{ inputs, ... }:
{
  perSystem =
    ...
    in
    {
      checks.health-sentinel = pkgs.runCommand "health-sentinel-check" {
        nativeBuildInputs = [ pkgs.coreutils pkgs.findutils pkgs.jq pkgs.gawk ];
      } ''
        ...
        {"captures":[{"name":"fixture","path":"$TMPDIR/capture","expectedCadenceSeconds":60}],"mounts":[{"path":"/fixture","warnPct":80,"failPct":95}],"observedServices":[{"manager":"system","unit":"fixture.service"}]}
        EOF_INVENTORY
