{ inputs, ... }:
{
  perSystem = { system, ... }:
    let
      ...
      data = pkgs.writeText "agent-environment-fixture.json" (builtins.toJSON {
        profiles = [
          { name = "full"; client = "codex"; tiers = [ "remote-core" ]; servers = [ "context7" ]; }
        ];
        servers = [
          { name = "context7"; tier = "remote-core"; transport = "http"; url = "https://mcp.context7.com/mcp"; clients = [ "codex" ]; }
        ];
      ...
      agents = pkgs.runCommand "agent-environment-fixture-agents" { } ''
        ...
        EOF
      '';
      rendered = pkgs.runCommand "agent-environment-fixture-rendered" {
        nativeBuildInputs = [ pkgs.bash pkgs.coreutils pkgs.findutils pkgs.gawk pkgs.jq pkgs.ripgrep ];
      } ''
    ...
    in
    {
      checks.agent-environment-doc = pkgs.runCommand "agent-environment-doc-check" {
        inherit rendered;
      } ''
        test -s $rendered
        touch $out
