...
- **All product/project code lands via PRs** to default. No direct pushes to
  ...
  **Sinnix and Lynchpin are the exceptions:** Sinnix is the operator's live
  system-configuration repo and Lynchpin is the operator's personal
  analysis hub; both are operated solo and may be committed and pushed
  directly on `master` after local verification (and, for Sinnix,
  successful deployment). Still write navigable commit messages.
...
surface. Sinnix and Lynchpin are intentionally operated directly on `master`;
do not invent a branch/PR boundary there unless explicitly requested.
