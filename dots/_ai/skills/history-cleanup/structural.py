...
def git(repo: Path, *args: str, text: bool = True, check: bool = True, input_data: str | bytes | None = None) -> subprocess.CompletedProcess[Any]:
...
def normalize_glob_lists(paths: list[str] | None = None, globs: list[str] | None = None) -> tuple[list[str], list[str]]:
...
BRACE_RENAME_RE = re.compile(r"^(?P<prefix>.*)\{(?P<old>[^{}]*) => (?P<new>[^{}]*)\}(?P<suffix>.*)$")
...
def commit_meta(repo: Path, sha: str) -> dict[str, Any]:
    ...
    commit_sha, parents, author_name, author_email, author_date_iso, committer_name, committer_email, committer_date_iso, message = parts[:9]
    return {
...
def operation_is_empty_band_noop(operation: dict[str, Any]) -> bool:
    source_indices = {int(value) for value in operation.get("source_band_selection_indices") or []}
...
def head_order(repo: Path, ref: str = "HEAD") -> tuple[list[str], dict[str, int]]:
    shas = [line.strip() for line in git_stdout(repo, "rev-list", "--reverse", "--topo-order", ref).splitlines() if line.strip()]
...
def compose_message(subject: str | None, message: str | None, body_lines: list[str] | None = None) -> str:
...
def discover_default_inputs(launch_pack_dir: Path) -> dict[str, Any]:
    return {
        "primary_packs": [
            launch_pack_dir / "agent-work" / "wave4-final" / "current-executable-pack.json",
            launch_pack_dir / "agent-work" / "final-residue" / "wave2-manual-executable-specs.json",
            launch_pack_dir / "agent-work" / "final-residue" / "older-manual-residue.json",
        ],
        "fallback_packs": [
            launch_pack_dir / "deterministic-wave2-execution-plan.json",
            launch_pack_dir / "agent-work" / "first738-splits" / "deterministic-split-proposal.json",
            launch_pack_dir / "agent-work" / "final-residue" / "wave3-deterministic-split-proposal.json",
        ],
        "supersession_files": [
...
def superseded_ids(paths: list[Path]) -> tuple[set[str], dict[str, str]]:
    ...
    for path in paths:
        if not path.exists():
            continue
        payload = load_json(path)
        for row in payload.get("supersessions", []) if isinstance(payload, dict) else []:
...
def expand_scaffold_assignment(assignment: dict[str, Any], scaffold_path: Path | None) -> tuple[list[str], list[str], list[str]]:
...
def normalize_child_units(
    ...
    local_selection_indices = [int(value) for value in (child.get("source_selection_indices") or selection_indices)]
    if child.get("selection_mode") == "whole_selection" and len(local_selection_indices) == 1:
        return [{"kind": "whole_selection", "namespace": namespace, "selection_index": local_selection_indices[0]}]
    if isinstance(child.get("source_assignment"), dict):
        include_paths, include_globs, exclude_paths = expand_scaffold_assignment(child["source_assignment"], scaffold_path)
    else:
        include_paths, include_globs = normalize_glob_lists(child.get("include_paths") or [], child.get("include_path_globs") or [])
        exclude_paths = list(child.get("exclude_paths") or [])
    if not include_paths and not include_globs and len(local_selection_indices) == 1 and not exclude_paths:
        return [{"kind": "whole_selection", "namespace": namespace, "selection_index": local_selection_indices[0]}]
...
def fragment_registry(
    ...
    def register(namespace: str, atom_id: str | None, selection_index: int | None, fragment_index: int | None, fragment: dict[str, Any]) -> None:
        if atom_id:
            registry[(namespace, atom_id)] = fragment
        if selection_index is not None and fragment_index is not None:
            fragment_index_registry[(namespace, selection_index, fragment_index)] = fragment
    ...
    for spec in primary_specs:
        ...
        for atom in spec.get("source_atoms") or []:
            if not isinstance(atom, dict):
                continue
            if not atom.get("commit_message") and not atom.get("commit_subject") and not atom.get("source_selection_indices"):
                continue
            if not atom.get("commit_message") and not atom.get("commit_subject"):
                continue
            atom_selection_indices = [int(value) for value in (atom.get("source_selection_indices") or selection_indices)]
            units = normalize_child_units(namespace, atom_selection_indices, atom)
            fragment = {
                "origin": spec["spec_id"],
                "namespace": namespace,
                "units": units,
                "commit_message": compose_message(atom.get("commit_subject"), atom.get("commit_message"), atom.get("commit_body_lines")),
            }
            atom_id = atom.get("atom_id")
            register(namespace, atom_id, atom_selection_indices[0] if len(atom_selection_indices) == 1 else None, None, fragment)
        if spec["kind"] == "split":
            for child_index, child in enumerate(spec["target_commits"], 1):
                ...
                fragment = {
                    ...
                    "namespace": namespace,
                    "units": units,
                    "commit_message": compose_message(child.get("commit_subject"), child.get("commit_message"), child.get("commit_body_lines")),
                }
                register(namespace, atom_id, selection_indices[0] if len(selection_indices) == 1 else None, child_index, fragment)
    ...
    for spec in fallback_specs:
        ...
        if spec_type == "wave2_execution_plan":
            ...
            for child_index, child in enumerate(spec["target_commits"], 1):
                atom_id = child.get("atom_id")
                units = normalize_child_units(namespace, spec["source_selection_indices"], child)
                fragment = {
                    "origin": spec["spec_id"],
                    "namespace": namespace,
                    "units": units,
                ...
                register(namespace, atom_id, spec["source_selection_indices"][0], child_index, fragment)
            ...
            scaffold_path = Path(spec.get("scaffold_path")) if spec.get("scaffold_path") else None
            for child_index, child in enumerate(spec["target_commits"], 1):
                ...
                fragment_index_registry[(namespace, selection_index, child_index)] = fragment
...
def load_fallback_specs(paths: list[Path]) -> list[dict[str, Any]]:
    ...
    for path in paths:
        ...
        if path.name == "deterministic-wave2-execution-plan.json":
            ...
            for item in payload.get("specs", []):
                if isinstance(item, dict) and isinstance(item.get("resolved_selection"), dict):
                    spec = dict(item)
...
def flatten_ordered_selection_indices(target_commits: list[dict[str, Any]]) -> list[int]:
    ordered: list[int] = []
...
def current_selection_order(namespace: str, selection_indices: list[int], selection_maps: dict[str, dict[int, str]], order_index: dict[str, int]) -> list[int]:
    ...
    for path in paths:
        for item in load_specs(path):
            if item.get("executability") != "executable":
                continue
            spec = dict(item)
            if "source_selection_indices" not in spec or not spec.get("source_selection_indices"):
                ...
                if not selection_indices:
                    for row in resolved:
                        ...
                        if isinstance(selection_range, list) and len(selection_range) == 2:
...
def overlap_components(specs: list[dict[str, Any]]) -> list[list[dict[str, Any]]]:
    ...
    while unresolved:
        ...
        while changed:
            ...
            current_indices = {index for item in component for index in item.get("source_selection_indices") or []}
            for candidate in unresolved:
                if candidate["source_namespace"] in current_namespace and current_indices & set(candidate.get("source_selection_indices") or []):
                    component.append(candidate)
                    current_indices.update(candidate.get("source_selection_indices") or [])
                    changed = True
                else:
...
def resolve_atom_reference(
    ...
    if kind == "selection":
        ...
        return [{"kind": "whole_selection", "namespace": namespace, "selection_index": selection_index}]
    if kind == "range":
        start = int(atom_ref["start"])
        end = int(atom_ref["end"])
        return [{"kind": "whole_selection", "namespace": namespace, "selection_index": value} for value in range(start, end + 1)]
    if kind == "split_fragment":
        ...
        if not fragment:
            raise KeyError(f"missing split fragment {namespace}:{selection_index}:{fragment_index}")
        return [dict(unit) for unit in fragment["units"]]
...
def child_units_from_spec(
    ...
    if child.get("ordered_source_selection_indices"):
        return [{"kind": "whole_selection", "namespace": namespace, "selection_index": int(value)} for value in child["ordered_source_selection_indices"]]
...
def merge_child_encodes_reorder(merge_spec: dict[str, Any], reorder_spec: dict[str, Any]) -> bool:
    ...
    for child in merge_spec["target_commits"]:
        ...
        for index in range(0, len(merge_sequence) - len(reorder_sequence) + 1):
            if merge_sequence[index : index + len(reorder_sequence)] == reorder_sequence:
                return True
    return False


def reorder_is_noop(spec: dict[str, Any], selection_maps: dict[str, dict[int, str]], order_index: dict[str, int]) -> bool:
    flattened = flatten_ordered_selection_indices(spec["target_commits"])
    if not flattened:
        return False
    current = current_selection_order(spec["source_namespace"], flattened, selection_maps, order_index)
...
def build_primary_operations(
    ...
    for spec in primary_specs:
        if spec["kind"] == "reorder" and reorder_is_noop(spec, selection_maps, order_index):
            skipped.append({"spec_id": spec["spec_id"], "reason": "noop_reorder_checkpoint"})
            continue
    ...
    for component in overlap_components(filtered_specs):
        ...
        merge_uses_split_ids = any(
            any(isinstance(atom, str) and atom in split_ids for atom in (spec.get("source_atoms") or []))
            or any(
                any(isinstance(atom, str) and atom in split_ids for atom in (child.get("ordered_source_atoms") or []))
        ...
        if split_specs and not merge_specs and not reorder_specs:
            ...
            combined["spec_id"] = "+".join(spec["spec_id"] for spec in sorted(split_specs, key=lambda item: min(item["source_selection_indices"])))
            combined["source_selection_indices"] = sorted({index for spec in split_specs for index in spec["source_selection_indices"]})
            combined["target_commits"] = [
                child
                for spec in sorted(split_specs, key=lambda item: (min(item["source_selection_indices"]), item["spec_id"]))
        ...
        if merge_specs and merge_uses_split_ids:
            ...
            skipped.extend({"spec_id": spec["spec_id"], "reason": "dependency_split_atom"} for spec in split_specs)
            skipped.extend({"spec_id": spec["spec_id"], "reason": "subsumed_by_merge_component"} for spec in reorder_specs)
            continue
        if merge_specs and reorder_specs:
            ...
            for merge_spec in merge_specs:
                ...
                for reorder_spec in reorder_specs:
                    if merge_spec["source_selection_indices"] == reorder_spec["source_selection_indices"] or merge_child_encodes_reorder(merge_spec, reorder_spec):
                        absorbed_reorders.add(reorder_spec["spec_id"])
            if kept_merges:
                selected_specs.extend(kept_merges)
                skipped.extend({"spec_id": spec_id, "reason": "absorbed_into_merge"} for spec_id in sorted(absorbed_reorders))
                for reorder_spec in reorder_specs:
                    ...
                    alternates.extend(
                        {
                            ...
                            "primary_split_ids": [spec["spec_id"] for spec in split_specs],
                        }
            ...
            alternates.extend(
                {
                    ...
                    "primary_split_ids": [split_spec["spec_id"] for split_spec in split_specs],
                }
...
def source_selection_order_key(namespace: str, selection_index: int, selection_maps: dict[str, dict[int, str]], order_index: dict[str, int]) -> tuple[int, int]:
    sha = selection_maps[namespace][selection_index]
    return order_index[sha], selection_index


def spec_band_range(spec: dict[str, Any], selection_maps: dict[str, dict[int, str]], order_index: dict[str, int]) -> tuple[int, int]:
    namespace = spec["source_namespace"]
    positions = [
        source_selection_order_key(namespace, int(selection_index), selection_maps, order_index)[0]
...
def build_passthrough_child(namespace: str, selection_index: int, source_meta: dict[int, dict[str, Any]]) -> dict[str, Any]:
    meta = source_meta[selection_index]
    return {
        "kind": "passthrough",
        "commit_message": meta["message"].rstrip() + "\n",
        "units": [{"kind": "whole_selection", "namespace": namespace, "selection_index": selection_index}],
...
def child_commit_with_units(
    ...
    units.sort(key=lambda unit: source_selection_order_key(namespace, int(unit["selection_index"]), selection_maps, order_index))
    contributing = contributing_selection_indices(units)
    positions = [source_selection_order_key(namespace, index, selection_maps, order_index)[0] for index in contributing]
    single_selection_split_group = contributing[0] if len(contributing) == 1 and all(unit["kind"] != "whole_selection" or len(contributing) == 1 for unit in units) else None
    return {
        "kind": "rewritten",
        "commit_message": compose_message(child.get("commit_subject"), child.get("commit_message"), child.get("commit_body_lines")),
        "units": units,
...
def source_meta_for_namespace(repo: Path, namespace: str, selection_map: dict[int, str], order_index: dict[str, int]) -> dict[int, dict[str, Any]]:
...
def assign_child_dates(children: list[dict[str, Any]], source_meta: dict[int, dict[str, Any]]) -> None:
    ...
    for child in children:
        ...
        if child["kind"] == "passthrough":
            ...
            oldest = min(metas, key=lambda row: parse_datetime_iso(row["author_date_iso"]))
            newest = max(metas, key=lambda row: parse_datetime_iso(row["committer_date_iso"]))
...
def compile_operation(
    ...
    all_selection_indices = sorted({int(index) for spec in specs for index in spec.get("source_selection_indices") or []})
    positions = [source_selection_order_key(namespace, index, selection_maps, order_index)[0] for index in all_selection_indices]
    ...
    band_selection_indices.sort(key=lambda value: source_selection_order_key(namespace, value, selection_maps, order_index))
    children: list[dict[str, Any]] = []
    if all(spec["kind"] == "split" for spec in specs):
        ordered_specs = sorted(specs, key=lambda item: (spec_band_range(item, selection_maps, order_index)[0], item["spec_id"]))
        for spec_index, spec in enumerate(ordered_specs, 1):
            for child_index, child in enumerate(spec["target_commits"], 1):
                children.append(child_commit_with_units(spec, child, registry, selection_maps, order_index, order_hint=spec_index * 100 + child_index))
    else:
        ordered_specs = sorted(specs, key=lambda item: spec_band_range(item, selection_maps, order_index)[0])
        for spec_index, spec in enumerate(ordered_specs, 1):
            for child_index, child in enumerate(spec["target_commits"], 1):
                children.append(child_commit_with_units(spec, child, registry, selection_maps, order_index, order_hint=spec_index * 100 + child_index))
    consumed = {selection_index for child in children for selection_index in child["contributing_selection_indices"]}
    for selection_index in band_selection_indices:
        if selection_index not in consumed:
            children.append(build_passthrough_child(namespace, selection_index, source_meta_ns))
...
def compile_structural_plan(repo: Path, launch_pack_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    primary_specs = [spec for spec in primary_specs if spec["spec_id"] not in superseded and not spec.get("superseded_by")]
    ...
    selected_specs, alternates, skipped = build_primary_operations(primary_specs, selection_maps, order_index)
    source_meta: dict[str, dict[int, dict[str, Any]]] = {
        ...
        while True:
            ...
            for spec in remaining:
                if spec["source_namespace"] != seed["source_namespace"]:
                    ...
                    continue
                spec_start, spec_end = spec_band_range(spec, selection_maps, order_index)
    ...
    plan = {
        ...
        "superseded_specs": [{"spec_id": spec_id, "reason": superseded_reasons.get(spec_id)} for spec_id in sorted(superseded)],
    }
    return plan, summary
...
def source_sha_from_todo_line(repo: Path, stripped_line: str) -> tuple[str | None, str | None]:
    parts = stripped_line.split()
    if not parts:
        return None, None
    command = parts[0]
    sha_token: str | None = None
    if command in ("pick", "p", "reword", "r", "edit", "e", "squash", "s", "fixup", "f"):
        if len(parts) >= 2:
            sha_token = parts[1]
...
def patch_todo_cmd(args: argparse.Namespace) -> int:
    ...
    patched: list[str] = []
    cli_path = Path(args.cli_path).resolve() if args.cli_path else Path(__file__).resolve().with_name("cli.py")
    ...
    for line in lines:
        ...
        if resolved_source_sha and resolved_source_sha not in structural_band_shas and command in (
            "pick",
            "p",
            "reword",
            "r",
...
def unit_patch_bytes(repo: Path, unit: dict[str, Any], selection_maps: dict[str, dict[int, str]], source_meta: dict[str, dict[int, dict[str, Any]]]) -> bytes:
    namespace = unit["namespace"]
    selection_index = int(unit["selection_index"])
    sha = selection_maps[namespace][selection_index]
    parent_sha = source_meta[namespace][selection_index]["parent_sha"]
    if not parent_sha:
        raise ValueError(f"selection {namespace}:{selection_index} points at a root commit; structural runner does not support root rewrites")
    args = ["diff", "--binary", parent_sha, sha]
    if unit["kind"] == "partial_selection":
        pathspecs: list[str] = []
        include_paths, include_globs = normalize_glob_lists(unit.get("include_paths") or [], unit.get("include_path_globs") or [])
        ...
        if unit.get("exclude_paths"):
            if not pathspecs:
                pathspecs.append(".")
            pathspecs.extend(pathspec_from_exclude(path) for path in unit["exclude_paths"])
        if pathspecs:
...
def unit_changed_paths(repo: Path, unit: dict[str, Any], selection_maps: dict[str, dict[int, str]], source_meta: dict[str, dict[int, dict[str, Any]]]) -> tuple[str, str, list[str]]:
    namespace = unit["namespace"]
    ...
    if not parent_sha:
        raise ValueError(f"selection {namespace}:{selection_label} points at a root commit; structural runner does not support root rewrites")
    args = ["diff", "--name-only", "--no-renames", parent_sha, sha]
    pathspecs = unit_pathspecs(unit)
    if pathspecs:
        args.extend(["--", *pathspecs])
    changed_paths = [line.strip() for line in git_stdout(repo, *args).splitlines() if line.strip()]
...
def apply_unit_state(repo: Path, unit: dict[str, Any], selection_maps: dict[str, dict[int, str]], source_meta: dict[str, dict[int, dict[str, Any]]]) -> bool:
    sha, _parent_sha, changed_paths = unit_changed_paths(repo, unit, selection_maps, source_meta)
    ...
    for chunk in chunked(present_paths):
        git_stdout(repo, "restore", f"--source={sha}", "--staged", "--worktree", "--", *chunk)
    for chunk in chunked(absent_paths):
        git(repo, "rm", "-q", "-r", "-f", "--ignore-unmatch", "--", *chunk)
    return True


def ordered_child_units(operation: dict[str, Any], child: dict[str, Any]) -> list[dict[str, Any]]:
    source_order = {
        int(selection_index): position
        for position, selection_index in enumerate(operation.get("source_band_selection_indices") or [])
    }
    ...
    for original_position, unit in enumerate(child["units"]):
        ...
        if selection_index is None:
            ...
            continue
        ordered.append(((0, source_order.get(int(selection_index), len(source_order) + original_position), original_position), unit))
...
def apply_structural_op_cmd(args: argparse.Namespace) -> int:
    ...
    operation = next((item for item in plan.get("operations", []) if item["op_id"] == args.op_id), None)
    if not operation:
        raise ValueError(f"unknown operation id {args.op_id}")
    source_count = int(operation["source_commit_count"])
    actual_band = [line.strip() for line in git_stdout(repo, "rev-list", "--reverse", f"--max-count={source_count}", "HEAD").splitlines() if line.strip()]
    if len(actual_band) != source_count:
        ...
        raise ValueError(f"operation {args.op_id} expected {source_count} commits at HEAD tail, got {len(actual_band)}")
    expected_band = [str(value).strip() for value in operation.get("source_band_current_shas") or []]
    emit_journal_event(
    ...
    if expected_band:
        ...
        expected_fingerprints = [commit_identity_fingerprint(meta) for meta in expected_meta]
        actual_fingerprints = [commit_identity_fingerprint(meta) for meta in actual_meta]
    ...
    for child in operation["target_commits"]:
        ...
        for unit in ordered_child_units(operation, child):
            any_patch = apply_unit_state(repo, unit, selection_maps, source_meta) or any_patch
        if not any_patch:
            emit_journal_event(
            ...
            raise RuntimeError(f"operation {args.op_id} produced an empty child commit for message {child['commit_message'].splitlines()[0]}")
        ...
        try:
            subprocess.run(["git", "commit", "-F", str(message_path)], cwd=repo, check=True, text=True, capture_output=True, env=env)
            created_children += 1
        finally:
...
def load_excluded_op_ids(path: Path) -> list[str]:
    payload = load_json(path)
    if isinstance(payload, dict):
        values = payload.get("exclude") or payload.get("excluded_operations") or payload.get("op_ids") or []
...
def filter_structural_plan_cmd(args: argparse.Namespace) -> int:
    ...
    kept_operations = [operation for operation in original_operations if operation.get("op_id") not in excluded]
    removed_operations = [operation for operation in original_operations if operation.get("op_id") in excluded]
    ...
    summary = {
        ...
        "excluded_op_ids": [row["op_id"] for row in filtered_plan["excluded_operations"]],
    }
...
def auto_resolve_rebase_conflicts(repo: Path) -> bool:
    ...
    rebase_subject = ""
    if rebase_head:
        rebase_subject = git_optional_stdout(repo, "show", "-s", "--format=%s", rebase_head).strip()
    all_paths = sorted(stages_by_path.keys())
    path_fingerprint = hashlib.sha1("\n".join(all_paths).encode("utf-8")).hexdigest()[:16]
    add_paths: list[str] = []
    ...
    if unresolved_paths and test_lane_subject and all(path.startswith("test/") for path in unresolved_paths):
...
def run_structural_plan_cmd(args: argparse.Namespace) -> int:
    ...
    if args.journal_jsonl:
        initial_env["LYNCHPIN_HISTORY_JOURNAL_JSONL"] = str(Path(args.journal_jsonl).resolve())
    if args.conflict_ledger_jsonl:
        initial_env["LYNCHPIN_HISTORY_CONFLICT_LEDGER_JSONL"] = str(Path(args.conflict_ledger_jsonl).resolve())
    continue_env = os.environ.copy()
    continue_env["GIT_EDITOR"] = "true"
    if args.journal_jsonl:
        continue_env["LYNCHPIN_HISTORY_JOURNAL_JSONL"] = str(Path(args.journal_jsonl).resolve())
    if args.conflict_ledger_jsonl:
        continue_env["LYNCHPIN_HISTORY_CONFLICT_LEDGER_JSONL"] = str(Path(args.conflict_ledger_jsonl).resolve())
    command = ["git", "rebase", "-i", "--rebase-merges", "--reschedule-failed-exec", "--root"]
    env = initial_env
    emit_journal_event(
...
def verify_rollback_drill_cmd(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="history-cleanup-rollback-") as tmp:
        ...
        if backup_ref:
            ...
            try:
                result["checks"].append(
                    {
                        "kind": "backup_ref",
                        "restored_head": git_stdout(checkout_dir, "rev-parse", "HEAD").strip(),
                        "restored_tree": git_stdout(checkout_dir, "rev-parse", "HEAD^{tree}").strip(),
                    }
                )
            finally:
        ...
        if bundle:
            ...
            subprocess.run(["git", "clone", str(bundle), str(clone_dir)], check=True, text=True, capture_output=True)
            result["checks"].append(
                {
                    "kind": "bundle",
                    "restored_head": git_stdout(clone_dir, "rev-parse", "HEAD").strip(),
                    "restored_tree": git_stdout(clone_dir, "rev-parse", "HEAD^{tree}").strip(),
                }
            )
...
def register_structural_subcommands(subparsers: Any) -> None:
    ...
    compile_plan.add_argument("--repo", default=".", help="Repository root to compile against.")
    compile_plan.add_argument("--launch-pack-dir", required=True, help="Launch-pack directory containing translated selection maps and structural packs.")
    compile_plan.add_argument("--output-json", required=True, help="Output path for the compiled operations plan.")
    compile_plan.add_argument("--summary-json", help="Optional output path for the compact summary JSON.")
    ...
    patch_todo.add_argument("--repo", help="Repository root. Defaults to the repo recorded in the plan.")
    patch_todo.add_argument("--plan-json", required=True, help="Compiled structural plan JSON.")
    patch_todo.add_argument("--cli-path", help="Path to cli.py used in inserted exec commands.")
    ...
    apply_op.add_argument("--repo", default=".", help="Repository root being rewritten.")
    apply_op.add_argument("--plan-json", required=True, help="Compiled structural plan JSON.")
    ...
    preserve_committer.add_argument("--repo", default=".", help="Repository root being rewritten.")
    preserve_committer.add_argument("--source-sha", required=True, help="Original source commit SHA whose committer metadata should be preserved.")
    ...
    compare_trees.add_argument("--left-repo", required=True, help="Reference repo path.")
    compare_trees.add_argument("--right-repo", required=True, help="Candidate rewritten repo path.")
    compare_trees.add_argument("--left-ref", default="HEAD", help="Git ref to compare in the left repo.")
    compare_trees.add_argument("--right-ref", default="HEAD", help="Git ref to compare in the right repo.")
    compare_trees.add_argument("--sample-limit", type=int, default=50, help="Maximum sample entries to retain per category.")
    compare_trees.add_argument("--output-json", help="Optional output path for the scorecard JSON.")
    ...
    filter_plan.add_argument("--plan-json", required=True, help="Compiled structural plan JSON to filter.")
    filter_plan.add_argument("--exclude-op-id", action="append", default=[], help="Operation id to exclude. Repeatable.")
    filter_plan.add_argument("--exclude-op-list-json", help="JSON file containing operation ids to exclude.")
    filter_plan.add_argument("--output-json", required=True, help="Output path for the filtered plan JSON.")
    filter_plan.add_argument("--summary-json", help="Optional output path for the filtered-plan summary JSON.")
    ...
    run_plan.add_argument("--repo", default=".", help="Repository root being rewritten.")
    run_plan.add_argument("--plan-json", required=True, help="Compiled structural plan JSON.")
    run_plan.add_argument("--cli-path", help="Path to cli.py. Defaults to the portable history_cleanup cli.py.")
    run_plan.add_argument("--journal-jsonl", help="Optional JSONL journal path for structural replay progress.")
    run_plan.add_argument("--conflict-ledger-jsonl", help="Optional JSONL ledger path for conflict classifications and resolutions.")
    ...
    rollback_drill.add_argument("--repo", default=".", help="Repository root whose backups are being checked.")
    rollback_drill.add_argument("--backup-ref", help="Local backup ref to restore in a disposable worktree.")
    rollback_drill.add_argument("--bundle", help="Git bundle path to clone and verify.")
    rollback_drill.add_argument("--output-json", help="Optional output path for the rollback verification JSON.")
