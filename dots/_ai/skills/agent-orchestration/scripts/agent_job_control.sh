...
while [[ $# -gt 0 ]]; do
  case "$1" in
  --state-dir)
    state_dir="${2:?agent_job_control.sh: --state-dir requires a value}"
    shift 2
    ;;
  -h | --help)
    usage
...
interrupt)
  ...
  kill -0 "${pid}" 2>/dev/null || {
    echo "refusing stale job PID: ${pid}" >&2
    exit 1
  }
  [[ "$(proc_start "${pid}")" == "$(jq -r '.launcher.proc_start' "${manifest}")" ]] || {
    echo "refusing PID start-time mismatch for job ${job_id}" >&2
    exit 1
  }
  [[ -r ${proc_root}/${pid}/cgroup ]] || {
    echo "refusing unreadable cgroup for PID: ${pid}" >&2
    exit 1
  }
  ...
  if [[ -n ${actual_pid} ]]; then
    ...
    kill -0 "${actual_pid}" 2>/dev/null || {
      echo "refusing stale actual-agent PID: ${actual_pid}" >&2
      exit 1
    }
    ...
    [[ -r ${proc_root}/${actual_pid}/cgroup ]] || {
      echo "refusing unreadable actual-agent cgroup: ${actual_pid}" >&2
      exit 1
    }
