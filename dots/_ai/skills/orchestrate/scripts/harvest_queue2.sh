...
fetch_locked || { echo "HARVEST-FAIL fetch"; exit 2; }
...
run_gate || { echo "HARVEST-FAIL quick gate — see $QLOG"; exit 3; }
exec {slot_fd}>&-
...
if [ "$(git rev-parse origin/master)" != "$GATED_AT" ]; then
  ...
  run_gate || { echo "HARVEST-FAIL quick gate after master moved — see $QLOG"; exit 3; }
fi
