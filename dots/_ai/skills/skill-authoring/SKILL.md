...
2. Write a specific frontmatter `name` and routing description of at most 35 words, then keep the main file below 500 lines.
3. Add only directly useful references, scripts, or assets. Check every relative link.
4. Validate structure with `scripts/validate_skill.py <skills-root>`. The validator checks frontmatter, duplicate names, description length, file size, and references. It does not judge prose with magic-phrase rules.
