...
while [[ $# -gt 0 ]]; do
  case $1 in
    --repo) repo=$2; shift 2 ;;
    --out) out=$2; shift 2 ;;
    --conflict) conflicts+=("$2"); shift 2 ;;
    -h|--help) usage; exit 0 ;;
    *) usage; exit 64 ;;
  esac
done
[[ -n $repo && -n $out ]] || { usage; exit 64; }
repo=$(git -C "$repo" rev-parse --show-toplevel)
[[ $out == /* ]] || { echo 'output must be absolute' >&2; exit 64; }
...
for file in "${conflicts[@]}"; do
  [[ -f $file ]] || { echo "conflict file is not regular: $file" >&2; exit 1; }
