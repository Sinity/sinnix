...
reusable methodology for the Codex `history-cleanup` skill rather than living
...
The reusable toolkit and the project-specific run corpora should live together
under `sinnix/dots/codex/skills/history-cleanup/`.

Recommended split:

- toolkit code, schemas, methodology, templates:
  - `history_cleanup/`
- project-specific durable run corpora:
  - `history_cleanup/project-runs/<project>/`
...
but the canonical worked record should sit beside the toolkit, not be stranded
inside repo-local scratch.
...
- prefer `24-48` owned commits per worker
...
- `32` owned commits per worker is a good default when workers must read full
  patches and nearby context
- `48` owned commits is acceptable for lighter bands
...
- `sinity-lynchpin/dots/codex/skills/history-cleanup/`
...
1. `prepare-wave`
   carve a wave into worker ranges and input manifests
2. `build-review-bundles`
   generate filtered patch bundles and surface metadata for workers
3. workers read and emit proposal JSON
4. `normalize-wave`
   convert heterogeneous worker outputs into one canonical corpus
5. `build-rewrite-map`
