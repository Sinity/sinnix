...
def main():
    ...
    if total > 0:
        ...
        print(
            f"Personal: {format_duration(personal_time):>15} ({personal_time / total * 100:>5.1f}%)"
        )
        print(
            f"Other:    {format_duration(other_time):>15} ({other_time / total * 100:>5.1f}%)"
        )
        ...
        if work_time > 0:
            ...
            if ratio < 0.3:
                print(
                    "⚠️  Warning: Low personal time. Consider better work-life balance."
                )
            elif ratio > 1.5:
