...
case "$cmd" in
  ...
  binds)
    ...
    while [[ $# -gt 0 ]]; do
      case "$1" in
        --json) json=1; shift ;;
        --grep) pattern="${2:?missing pattern}"; shift 2 ;;
  ...
  dispatch)
    [[ $# -ge 1 ]] || { echo "dispatch requires dispatcher name" >&2; exit 2; }
    hyprctl dispatch "$@"
    ;;

  keyword)
    [[ $# -ge 2 ]] || { echo "keyword requires name and value" >&2; exit 2; }
    name="$1"; shift
    value="$*"
  ...
  batch)
    [[ $# -eq 1 ]] || { echo "batch requires one quoted command string" >&2; exit 2; }
    hyprctl --batch "$1"
    ;;
