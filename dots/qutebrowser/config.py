...
# When qutebrowser loads config.py it injects `config` (ConfigAPI) and `c`
# (ConfigContainer). When the module is imported outside of that path (hook
# metadata), recreate equivalent objects from the global config state.
def _ensure_config_objects():
  global config, c  # noqa: PLW0603

  if "config" in globals() and hasattr(config, "load_autoconfig"):
    return

  from qutebrowser.config import config as config_state
  from qutebrowser.config import configfiles as configfiles_mod

  if config_state.instance is None or config_state.key_instance is None:
    raise RuntimeError("qutebrowser config state unavailable during import")

  config = configfiles_mod.ConfigAPI(
    config_state.instance,
    config_state.key_instance,
    warn_autoconfig=False,
  )
  c = config_state.ConfigContainer(config_state.instance, configapi=config)


_ensure_config_objects()
...
# Appearance --------------------------------------------------------------- #
c.window.title_format = "{perc}{audio}{current_title} — {host}"
...
if not globals().get("_SINNIX_QUTE_HOOK_REGISTERED", False):
    _SINNIX_QUTE_HOOK_REGISTERED = True
