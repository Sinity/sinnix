...
def test_a_declared_pool_runs_the_child_as_a_service_that_exits_with_its_cgroup(
    ...
    unit = scope_unit_for(launch, "pytest")
...
def test_a_launch_input_without_a_pool_is_contained_by_its_pueue_group(
    ...
    assert f"--unit={scope_unit_for(launch, 'pytest')}" in fake_systemd.run_argv()
...
def test_a_single_slot_pool_held_by_a_running_task_refuses_the_run(
    ...
    fake_systemd.units.write_text(
        f"{scope_unit_for('/inputs/other.json', 'pytest')} loaded active running\n"
    )
    launch = write_launch(tmp_path, pool="pytest")
...
def test_a_unit_no_queued_task_owns_also_holds_the_slot(
    ...
    fake_systemd.units.write_text(
        "agentctl-pytest-stray-0123456789ab.service loaded active running\n"
    )
    launch = write_launch(tmp_path, pool="pytest")
...
def test_a_unit_whose_task_is_terminal_is_an_orphan_and_is_settled(
    ...
    unit = scope_unit_for("/inputs/other.json", "pytest")
    fake_systemd.units.write_text(f"{unit} loaded active running\n")
    launch = write_launch(tmp_path, pool="pytest")
...
def test_a_multi_slot_pool_is_not_guarded(
    ...
    fake_systemd.units.write_text(
        "agentctl-agent-other-0123456789ab.service loaded active running\n"
    )
...
def test_the_unit_description_is_the_pueue_task_id(
    tmp_path: Path, fake_systemd: FakeSystemd, fake_pueue: FakePueue
) -> None:
    launch = write_launch(tmp_path, pool="pytest")
    ...
    assert main([str(launch)]) == 0
    assert f"--description={task_id}" in fake_systemd.run_argv()
