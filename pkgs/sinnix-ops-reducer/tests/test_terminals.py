...
def test_named_keys_reject_anything_not_in_the_quick_action_set() -> None:
    ...
    assert set(terminals.NAMED_KEYS) == {"enter", "ctrl-c", "ctrl-d", "tab", "up", "escape"}
...
def test_history_for_joins_by_pid_and_window_id(tmp_path: Path) -> None:
    ...
    (tmp_path / "2026-08-18T00-00-00-prime-pid111-win222-bash.meta.json").write_text(json.dumps(matching))
    ...
    (tmp_path / "2026-08-18T00-00-01-prime-pid111-win333-zsh.meta.json").write_text(json.dumps(other))
...
def test_history_for_a_directory_that_does_not_exist_yet_is_empty(tmp_path: Path) -> None:
...
def test_route_regexes_extract_pid_and_window_id(path: str, expect_pid: str, expect_win: str) -> None:
...
def test_history_file_route_only_matches_dot_ansi_names() -> None:
    ...
    assert terminals.HISTORY_FILE_RE.match("/terminals/v1/windows/1/2/history/../../etc/passwd") is None
    assert terminals.HISTORY_FILE_RE.match("/terminals/v1/windows/1/2/history/x.txt") is None
...
def test_the_terminal_index_page_is_reachable_over_the_shared_listener(hub_server: str) -> None:
...
def test_windows_list_is_json_on_the_shared_listener(hub_server: str, monkeypatch) -> None:
...
def test_send_dispatches_text_and_key_to_the_kitty_helpers(hub_server: str, monkeypatch) -> None:
    calls = []
    monkeypatch.setattr(terminals, "send_text", lambda pid, win, text: calls.append(("text", pid, win, text)) or True)
    monkeypatch.setattr(terminals, "send_key", lambda pid, win, key: calls.append(("key", pid, win, key)) or True)
...
def test_send_answers_404_when_the_kitty_helper_refuses(hub_server: str, monkeypatch) -> None:
