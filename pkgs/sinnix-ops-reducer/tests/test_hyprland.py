from sinnix_ops_reducer.hyprland import HyprlandState, reconcile_activewindow, reduce_socket_event
...
def test_restart_reconciliation_restores_fullscreen_and_static_state() -> None:
    state = HyprlandState()
    snapshot = reconcile_activewindow(state, {"fullscreen": 2, "class": "mpv"}, now=100.0)

    assert snapshot == {"fullscreen_game": True, "static_content": True, "diagnostics": 0}
