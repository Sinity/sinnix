...
def test_long_afk_produces_reference_only_anchor_and_deduplicates() -> None:
    ...
    event = {
        ...
        "resumed_at": "2026-08-07T12:00:00Z",
        "observe": {
            "focused_project": "sinnix",
            "recent_commits": ["git:abc"],
            "polylogue": None,
        },
    }
...
def test_short_afk_is_ignored_and_anchor_expires() -> None:
    ...
    assert (
        reduce_anchor_event(
            {
                "started_at": "2026-08-07T11:40:00Z",
                "resumed_at": "2026-08-07T12:00:00Z",
            },
            now,
        )
    ...
    anchor = reduce_anchor_event(
        {"started_at": "2026-08-07T11:00:00Z", "resumed_at": "2026-08-07T12:00:00Z"},
        now,
    )
    assert (
        expire_anchor(anchor, datetime(2026, 8, 7, 13, 1, tzinfo=timezone.utc)) is None
    )
...
def test_reducer_consumes_resume_event_once(tmp_path) -> None:
    ...
    event_path.write_text(
        json.dumps(
            {"started_at": started_at.isoformat(), "resumed_at": resumed_at.isoformat()}
        )
    )
    reducer = Reducer(
        tmp_path / "snapshot.json", tmp_path / "token", lambda: {"schema": "observe"}
    )
