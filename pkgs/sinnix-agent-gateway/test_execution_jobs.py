...
def test_public_v2_job_verbs_dispatch_catalog_bound_owner(
    ...
    monkeypatch.setattr(
        runtime.beads,
        "change",
        lambda _project, operation, parameters, **kwargs: (
            claim_calls.append(
    ...
    assert (
        started["data"]["claim_ref"]
        == "sinnix://projects/fixture/beads/fixture-1/claims/" + "d" * 64
    )
...
def test_bead_review_and_evidence_close_require_bound_successful_job(
    ...
    monkeypatch.setattr(
        ...
        "change",
        lambda _project, operation, parameters, **kwargs: (
            changes.append({"operation": operation, "parameters": parameters, **kwargs})
            or {"after": bead}
        ),
    )
    monkeypatch.setattr(
        runtime.projects,
        "checkout",
        lambda *_args: {"checkout": {"checkout_id": "default", "head": "d" * 40}},
    )
    ...
    with pytest.raises(ProtocolError, match="current checkout"):
        runtime.v2_beads_change(
            ...
            parameters={
                ...
                "residuals": [],
                "evidence_refs": [
                    f"sinnix://jobs/{job_id}",
                    f"sinnix://jobs/{job_id}/artifacts/result",
                ],
                "job_ref": f"sinnix://jobs/{job_id}",
                "code_revision": "c" * 40,
                "task_revision": "a" * 64,
                "task_etag": "b" * 64,
            },
    ...
    closed = runtime.v2_beads_change(
        reference=bead["ref"],
        operation="close_with_evidence",
        parameters={
            "verdict": "accepted",
    ...
    assert review["revision_mismatch"] == {
        "task_revision": True,
        "task_etag": True,
        "code_revision": True,
    }
    ...
    with pytest.raises(ProtocolError, match="cannot close"):
        runtime.v2_beads_change(
            ...
            parameters={
                "verdict": "accepted",
                "residuals": [],
                "evidence_refs": [f"sinnix://jobs/{job_id}"],
                "job_ref": f"sinnix://jobs/{job_id}",
                "code_revision": "d" * 40,
                "task_revision": "a" * 64,
                "task_etag": "b" * 64,
...
def test_agent_control_bead_scope_requires_matching_current_assignment(
    ...
    daemon.responses["job.agent.start"] = {
        "job_id": "4a42f848-9057-4cef-9d27-80a022c0e16f",
        "state": {"phase": "running"},
    }
    monkeypatch.setattr(runtime.beads, "get", lambda *_args, **_kwargs: bead)
    monkeypatch.setattr(
        runtime.projects,
        "checkout",
        lambda *_args: {"checkout": {"checkout_id": "default", "head": "c" * 40}},
    )
    ...
    started = runtime.v2_run_for_bead(
        ...
        instructions="private launch instruction",
        backend="codex",
        model="gpt-5.6-terra",
        reasoning_effort="high",
        timeout_seconds=60,
    ...
    assert (
        daemon.calls[-1].arguments["bead_binding"]["assignment_ref"] == assignment_ref
    )
    assert daemon.calls[-1].arguments["bead_binding"]["write_scope"] == [
        "pkgs/sinnixd/"
    ]
    assert (
        "private launch instruction"
        not in daemon.calls[-1].arguments["bead_binding"].values()
    )
...
def test_agent_control_bead_review_authorizes_assignment_before_bead_read(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: pytest.fail(
            "bead owner was read before assignment authorization"
        ),
    )
...
def test_v2_jobs_query_bounds_daemon_job_list_and_preserves_job_refs(
    tmp_path: Path,
) -> None:
    runtime, daemon = runtime_with_daemon(tmp_path, "observer")
    daemon.responses = {
...
def test_v2_job_routes_preserve_principal_policy(tmp_path: Path) -> None:
    observer, daemon = runtime_with_daemon(tmp_path, "observer", max_result_bytes=1_024)
