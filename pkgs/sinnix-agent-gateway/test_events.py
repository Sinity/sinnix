...
from sinnix_agent_gateway.events import (
    MAX_EVENT_PROJECTS,
    EventCursorError,
...
class FakeProjects:
    def __init__(self, project: Path):
        self.config = type(
            "Config", (), {"projects": {"fixture": ProjectConfig("fixture", project)}}
        )()
        self.revision = "git-a"
...
def service(
    tmp_path: Path,
) -> tuple[NormalizedEventService, FakeProjects, FakeBeads, AuditService]:
...
def test_events_normalize_owner_revisions_without_fabricating_exact_events(
    tmp_path: Path,
) -> None:
    ...
    assert {row["kind"] for row in first["events"]} >= {
        "gateway_receipt",
        "git_revision",
        "owner_revision",
        "runtime_transition",
    ...
    assert all(
        ...
        if row["kind"] in {"gateway_receipt", "runtime_transition"}
    )
    assert all(
        row["exact"] is False
        for row in first["events"]
        if row["kind"] in {"git_revision", "owner_revision"}
    )
    assert all(
        "ref" not in row
        for row in first["events"]
        if row["kind"] == "runtime_transition"
    )
    ...
    assert {row["kind"] for row in third["events"]} >= {
        "git_revision",
        "owner_revision",
    }
...
def test_event_cursor_state_and_runtime_continuation_preserve_rows(
    tmp_path: Path,
) -> None:
    events, _projects, _beads, _audit = service(tmp_path)
    events.transitions_path.write_text(
        "".join(
            f'{{"schema":"sinnix-health-transition-v1","event_id":"row-{i}","data":"{i}"}}\n'
            for i in range(4)
        )
    )
    seen: list[str] = []
    ...
    for _ in range(8):
        ...
        seen.extend(
            ...
            if row["event_id"].startswith("row-")
        )
        cursor = page["next_cursor"]
...
def test_event_cursor_state_is_bounded_independently_of_job_population(
    ...
    events, _projects, _beads, _audit = service(tmp_path)
    jobs = [
        {"job_id": f"job-{index}", "state": {"phase": "running"}}
        for index in range(10_000)
    ]
    events.jobs = lambda _limit, _cursor: {
        "jobs": jobs,
        "snapshot": {"ordering": "created_at_desc_job_id_desc", "ceiling": ["", ""]},
    }
...
def test_oversized_runtime_row_is_compacted_without_advancing_past_next_row(
    ...
    for _ in range(10):
        ...
        runtime = next(
            (row for row in first["events"] if row["data"].get("truncated") is True),
            None,
        )
    ...
    assert [
        row["event_id"] for row in second["events"] if row["event_id"] == "after"
    ] == ["after"]
