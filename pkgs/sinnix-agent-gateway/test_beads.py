...
def beads_service(
    ...
    runner.write_text(
        ...
        "state=json.loads(pathlib.Path(state_path).read_text()) if pathlib.Path(state_path).exists() else {'writes': 0, 'created': 0, 'revision': 0, 'metadata': {'fixture-1': {'unrelated': 'kept'}}}\n"
        "args=sys.argv[1:]\n"
        ...
        "    state['writes'] += 1; state['created'] += 1; state['revision'] += 1; pathlib.Path(state_path).write_text(json.dumps(state)); print(json.dumps({'id':f'fixture-created-{state[\"created\"]}','title':'created','status':'open'}))\n"
        ...
        "elif 'show' in args:\n"
        "    issue_id=next((x for x in args if x.startswith('fixture-')), 'fixture-1')\n"
        "    print(json.dumps({'id':issue_id,'title':'fixture','status':'open','notes':'long existing notes','labels':['lane:gateway'],'metadata':state['metadata'].get(issue_id,{})}))\n"
        "elif 'dep' in args:\n"
        "    if '--readonly' not in args and any(item in args for item in ('add','remove')): state['writes'] += 1; state['revision'] += 1; pathlib.Path(state_path).write_text(json.dumps(state))\n"
        ...
        "        if '--metadata' in args: state['metadata'].setdefault('fixture-1',{}).update(json.loads(args[args.index('--metadata')+1]))\n"
        "        for index, value in enumerate(args):\n"
        "            if value == '--unset-metadata': state['metadata'].setdefault('fixture-1',{}).pop(args[index+1],None)\n"
        "        state['writes'] += 1; state['revision'] += 1; pathlib.Path(state_path).write_text(json.dumps(state))\n"
    ...
    cfg = GatewayConfig(
        ...
        projects={
            ...
            "other": ProjectConfig(
                ...
                task_authority=TaskAuthorityConfig(
                    owner="beads",
                    workspace=other_project / ".beads",
                    database=other_project / ".beads" / "dolt",
                ),
            ),
        },
...
def test_query_normalizes_project_qualified_resources_and_snapshot_pages(
    tmp_path: Path,
) -> None:
...
def test_ready_query_requests_issue_rows_without_unbounded_explanation(
    tmp_path: Path,
) -> None:
    beads, log = beads_service(tmp_path)
...
def test_preview_never_writes_and_apply_protects_notes_by_default(
    ...
    preview = beads.change(
        "fixture",
        "update",
        {"id": "fixture-1", "patch": {"notes": {"text": "new context"}}},
        mode="preview",
    )
    ...
    assert any(
        "update" in command and "--append-notes" in command for command in commands(log)
    )
    with pytest.raises(BeadsError, match="stale"):
        beads.change(
            "fixture",
            "update",
            {"id": "fixture-1", "patch": {"notes": {"text": "x"}}},
            preview_digest="0" * 64,
        )


def test_replace_notes_requires_explicit_mode_and_cas_is_forwarded(
    tmp_path: Path,
) -> None:
    beads, log = beads_service(tmp_path)
    preview = beads.change(
        ...
        {
            "id": "fixture-1",
            "patch": {"notes": {"text": "intentional replacement", "mode": "replace"}},
        },
        mode="preview",
        preconditions={"expected_status": "open", "expected_assignee": None},
    )
    assert (
        "--notes" in preview["command"] and "--append-notes" not in preview["command"]
    )
    ...
    with pytest.raises(BeadsError, match="status no longer matches"):
        beads.change(
            "fixture",
            "update",
            {"id": "fixture-1", "patch": {"set": {"status": "closed"}}},
            mode="preview",
            preconditions={"expected_status": "closed"},
        )
...
def test_typed_mutations_and_owner_authority_are_enforced(tmp_path: Path) -> None:
    beads, log = beads_service(tmp_path)
    beads.change(
        "fixture", "dependency.add", {"id": "fixture-1", "depends_on": "fixture-2"}
    )
    beads.change(
        "fixture",
        "memory.remember",
        {"id": "fixture-1", "key": "gateway", "text": "fact"},
    )
...
def test_operator_close_requires_an_explicit_true_force_override(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(BeadsError, match="force must be true"):
        beads.change(
            "fixture", "close", {"id": "fixture-1", "force": False}, mode="preview"
        )
...
def test_every_declared_non_admin_mutation_has_preview_and_apply(
    ...
    applied = beads.change(
        "fixture", operation, parameters, preview_digest=preview["preview_digest"]
    )
...
def test_create_graph_uses_native_dry_run_and_cleans_its_input(tmp_path: Path) -> None:
    ...
    preview = beads.change(
        "fixture",
        "graph.create",
        {"graph": {"issues": [{"title": "one"}]}},
        mode="preview",
    )
    assert preview["native_validation"] == "dry_run"
    assert all(
        not path.exists()
        for path in (tmp_path / "state" / "beads-graph-inputs").glob("*")
    )
    assert any(
        "--graph" in command and "--dry-run" in command for command in commands(log)
    )
...
def test_changeset_preview_is_non_mutating_and_binds_created_beads(
    tmp_path: Path,
) -> None:
    beads, log = beads_service(tmp_path)
    actions = [
        ...
        },
        {
            "ref": "sinnix://projects/fixture",
            "operation": "create",
            "parameters": {"title": "child", "parent": "$parent"},
            "bind": "child",
        },
        {
            "ref": "sinnix://projects/fixture",
            "operation": "dependency.add",
            "parameters": {"id": "$child", "depends_on": "$parent"},
        },
    ]
    ...
    assert all(
        "create" not in command or "--dry-run" in command for command in commands(log)
    )
    applied = beads.changeset(
        actions, mode="apply", preview_digest=preview["preview_digest"]
    )
    assert [item["outcome"] for item in applied["outcomes"]] == [
        "applied",
        "applied",
        "applied",
    ]
    ...
    dependency = next(
        command for command in commands(log) if "dep" in command and "add" in command
    )
    assert "fixture-created-2" in dependency and "fixture-created-1" in dependency


def test_changeset_reports_failure_skips_and_never_sweeps_an_unrelated_writer(
    tmp_path: Path,
) -> None:
    beads, log = beads_service(tmp_path)
    actions = [
        ...
        },
        {
            "ref": "sinnix://projects/fixture",
            "operation": "create",
            "parameters": {"title": "force-fail"},
        },
        {
            "ref": "sinnix://projects/fixture",
            "operation": "create",
            "parameters": {"title": "skipped"},
        },
    ]
    ...
    runner = tmp_path / "bd"
    subprocess.run(
        [str(runner), "unrelated-write"], check=True, capture_output=True, text=True
    )
    ...
    with pytest.raises(BeadsError, match="stale"):
        beads.changeset(
            actions[:1], mode="apply", preview_digest=preview["preview_digest"]
        )
...
def test_changeset_rejects_cross_project_graph_edges_before_mutation(
    tmp_path: Path,
) -> None:
    beads, log = beads_service(tmp_path)
    with pytest.raises(BeadsError, match="cross-project"):
        beads.changeset(
            [
                ...
                {
                    "ref": "sinnix://projects/other",
                    "operation": "create",
                    "parameters": {"title": "second", "parent": "$first"},
                },
            ],
            mode="preview",
        )
...
def test_changeset_partitions_independent_projects_and_validates_every_precondition(
    tmp_path: Path,
) -> None:
    beads, _ = beads_service(tmp_path)
    actions = [
        {
            "ref": "sinnix://projects/fixture",
            "operation": "create",
            "parameters": {"title": "one"},
        },
        {
            "ref": "sinnix://projects/other",
            "operation": "create",
            "parameters": {"title": "two"},
        },
    ]
            ...
            [
                {
                    "ref": "sinnix://projects/fixture",
                    "operation": "create",
                    "parameters": {"title": "blocked"},
                },
                {
                    "ref": "sinnix://projects/fixture",
                    "operation": "create",
                    "parameters": {"title": "bad"},
                    "preconditions": {"expected_task_revision": "not-a-revision"},
                },
            ],
            mode="apply",
        )
...
def test_graph_changeset_is_owner_atomic_only_after_native_validation(
    tmp_path: Path,
) -> None:
    beads, log = beads_service(tmp_path)
    actions = [
        {
            ...
            "parameters": {
                "graph": {"issues": [{"title": "parent"}, {"title": "child"}]}
            },
        },
    ]
    preview = beads.changeset(actions, mode="preview")
...
def test_explicit_maintenance_operations_publish_a_deterministic_snapshot_receipt(
    tmp_path: Path,
) -> None:
    beads, log = beads_service(tmp_path)
    ...
    assert (
        second["publication"]["changed"] is False
        and second["publication"]["diff"] == ""
    )
    ...
    assert any(
        "backup" in command and "restore" in command and "fixture-backup" in command
        for command in commands(log)
    )
