...
def test_cli_defaults_to_the_deployed_local_estate_contract(
    ...
    assert (
        cli.parser().parse_args(["--config", str(explicit), "status"]).config
        == explicit
    )
    ...
    [
        ...
        (
            "change",
            {
                ...
                "ref": "sinnix://projects/fixture",
                "operation": "comment",
                "parameters": {"id": "fixture-1", "text": "fixture"},
                "idempotency_key": "cli-change",
            },
        ),
        (
            "operate",
...
def test_every_v2_verb_replays_through_the_matching_mcp_tool(
    ...
    response = anyio.run(
        cli_support.invoke_mcp, _config(tmp_path), "operator", verb, payload
    )
...
def test_catalog_display_exposes_schema_example_and_resource_completion() -> None:
    schema = cli_support.catalog_display(
        principal="operator", action_name="beads.change", schema=True
    )
    example = cli_support.catalog_display(
        principal="operator", action_name="beads.change", example=True
    )
    completion = cli_support.catalog_display(
        principal="operator", complete="sinnix://gateway/v2/actions/beads."
    )
...
def test_generated_cli_examples_are_replayable_against_a_fixture_server(
    ...
    assert [name for name, _ in calls] == [
        fixture["verb"] for fixture in fixtures["examples"]
    ]
