...
def _job(job_id: str, operation: str, phase: str, workspace: str, minutes_ago: int, *, terminal: bool = False) -> dict:
    created = (NOW - timedelta(minutes=minutes_ago)).isoformat()
    return {
        "job_id": job_id,
        "created_at": created,
        "spec": {"project_id": "polylogue", "kind": "declared-operation", "operation": operation,
                 "checkout": {"path": f"/realm/worktrees/{workspace}"}},
        "state": {"phase": phase, "terminal": terminal, "observed_at": NOW.isoformat(), "blocked_by": ["pool-workers"]},
    }


STATUS = {
    "project_id": "polylogue",
    "master_corpus": {"green": False, "red": 398, "passed": 20654, "head": "966b7e0fb065", "finished_at": (NOW - timedelta(minutes=10)).isoformat(), "job_id": "4f98e6cc"},
    "lanes_next": [
        {"workspace": "packet-polylogue-a", "bead": "polylogue-a", "next": {"kind": "park", "reason": "CI red on the PR"}, "pr": {"number": 7, "verdict": "ci-red"}, "receipt": {"flags": ["FLAG: x"]}},
        {"workspace": "packet-polylogue-b", "bead": "polylogue-b", "next": {"kind": "verify", "reason": "no receipt at head"}},
        {"workspace": "packet-polylogue-c", "bead": None, "next": {"kind": "idle", "reason": "dormant workspace"}},
    ],
    "errors": [],
}
...
def test_checks_name_every_stuck_thing_and_nothing_else() -> None:
    ...
    quiet = checks({**STATUS, "master_corpus": {"green": True, "red": 0, "passed": 1, "head": "x", "finished_at": NOW.isoformat()}, "lanes_next": []},
                   [], now=NOW, reactor_active=True, last_dispatch=NOW.isoformat())
    assert quiet == []


def test_overview_orders_lanes_by_urgency_and_hides_idle() -> None:
    text = render_overview(STATUS, [], now=NOW, reactor_active=True, last_dispatch=(NOW - timedelta(minutes=9)).isoformat())
    ...
    text = render_overview(STATUS, [ghost], now=NOW, reactor_active=True, last_dispatch=NOW.isoformat())
...
def test_lane_log_lists_jobs_and_events_in_time_order(tmp_path: Path) -> None:
    ...
    spool.write_text(json.dumps({"kind": "dispatch", "emitted_at": (NOW - timedelta(minutes=30)).isoformat(),
                                 "workspace": "packet-polylogue-a", "action": "verify", "reason": "no receipt at head"}) + "\n")
    jobs = [
        _job("22222222", "verify_affected", "succeeded", "packet-polylogue-a", 29, terminal=True),
    ...
    assert body.index("dispatch") < body.index("verify_affected") < body.index("harvest")
    assert "zzz" not in text and "… running for 5m" in text


def test_load_jobs_filters_by_project(tmp_path: Path) -> None:
    (tmp_path / "a.json").write_text(json.dumps(_job("a", "harvest", "running", "w", 1)))
