...
class TaskListCursorError(TaskError):
    ...
    def __init__(self, message: str, *, stale: bool = False):
        super().__init__(
            ErrorCode.STALE_CURSOR if stale else ErrorCode.INVALID_ARGUMENT, message
        )
...
class TaskAuthority:
    ...
    def load(cls, state_root: Path, project_id: str) -> TaskAuthority:
        ...
        try:
            ...
            raise TaskError(
                ErrorCode.OWNER_UNAVAILABLE, "task authority is not activated"
            ) from error
        except (OSError, json.JSONDecodeError) as error:
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task authority receipt is unavailable"
            ) from error
        expected = {
            "schema",
            "project_id",
        ...
        if (
            ...
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task authority receipt is malformed"
            )
        ...
        if (
            ...
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task authority verification is incomplete"
            )
        try:
            canonical_database = database.resolve(strict=True)
            source_database = Path(source_value)
            redirect_target = Path(
                (source_database.parent / "redirect").read_text().strip()
            ).resolve(strict=True)
        except (OSError, ValueError) as error:
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task authority cutover is ambiguous"
            ) from error
        if (
            source_database.exists()
            or source_database.is_symlink()
            or redirect_target != canonical_database.parent
        ):
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task authority cutover is ambiguous"
            )
...
class TaskMutationIdentity:
    ...
    @classmethod
    def create(
        cls, project_id: str, operation: str, task_id: str, idempotency_key: str
    ...
    def record_key(self) -> str:
        return self.digest(
            json.dumps(self.public(), sort_keys=True, separators=(",", ":"))
        ).removeprefix("sha256:")
...
class TaskMutationJournal:
    ...
    def load(
        self, identity: TaskMutationIdentity, arguments_sha256: str
    ) -> TaskMutationRecord | None:
        record = self._load(identity)
        if record is not None and record.arguments_sha256 != arguments_sha256:
            raise TaskError(
                ErrorCode.INVALID_ARGUMENT,
                "idempotency identity belongs to a different task mutation",
            )
        return record
    ...
    def create(
        ...
        if self._load(identity) is not None:
            raise TaskError(
                ...
                "task mutation journal changed during submission",
            )
        if len(tuple(self.records_root.glob("*.json"))) >= self.max_records:
            raise TaskError(
                ErrorCode.RESOURCE_EXHAUSTED, "task mutation journal is full"
            )
        try:
            intent = json.dumps(
                {"schema": 1, "command": list(command)},
                sort_keys=True,
                separators=(",", ":"),
            ).encode()
        except (TypeError, ValueError) as error:
            raise TaskError(
                ErrorCode.INVALID_ARGUMENT, "task mutation intent is invalid"
            ) from error
        if len(intent) > MAX_TASK_MUTATION_INTENT_BYTES:
            raise TaskError(
                ErrorCode.RESOURCE_EXHAUSTED,
                "task mutation intent exceeds the storage bound",
            )
        intent_sha256 = "sha256:" + hashlib.sha256(intent + b"\n").hexdigest()
        self._write(self._intent_path(identity), intent)
        record = TaskMutationRecord(
            identity, arguments_sha256, "pending", 0, intent_sha256, None, None
        )
        self.save(record)
        return record
    ...
    def save(self, record: TaskMutationRecord) -> None:
        ...
        if len(encoded) > MAX_TASK_MUTATION_RECORD_BYTES:
            raise TaskError(
                ErrorCode.RESOURCE_EXHAUSTED,
                "task mutation receipt exceeds the storage bound",
            )
    ...
    def records(self) -> tuple[TaskMutationRecord, ...]:
        try:
            ...
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation journal is unavailable"
            ) from error
        if len(paths) > self.max_records:
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation journal exceeds its bound"
            )
    ...
    def intent(self, record: TaskMutationRecord) -> tuple[str, ...]:
        try:
            ...
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation intent is missing"
            ) from error
        except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation intent is unavailable"
            ) from error
        if "sha256:" + hashlib.sha256(encoded).hexdigest() != record.intent_sha256:
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation intent is malformed"
            )
        command = (
            value.get("command")
        ...
        if (
            ...
            or len(command) > 32
            or any(
                not isinstance(item, str) or not item or len(item) > 32_000
                for item in command
            )
        ):
            raise TaskError(
    ...
    def dispatching(self, record: TaskMutationRecord) -> TaskMutationRecord:
        updated = replace(
            record, state="dispatching", attempts=record.attempts + 1, failure=None
        )
        self.save(updated)
        return updated

    def pending(
        self, record: TaskMutationRecord, error: TaskError
    ) -> TaskMutationRecord:
    ...
    def applied(
        ...
        *,
        ...
        try:
            ...
            raise TaskError(
                ErrorCode.RESULT_INVALID, "task backend returned invalid JSON"
            ) from error
        ...
        try:
            ...
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation intent cleanup failed"
            ) from error
        return updated
    ...
    def _read(self, path: Path) -> TaskMutationRecord:
        try:
            ...
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation journal is malformed"
            ) from error
        expected = {
            "schema",
            "identity",
    ...
    def _validate(record: TaskMutationRecord) -> None:
        ...
        if record.result is not None and (
            ...
            or set(record.result)
            not in ({"sha256", "bytes"}, {"sha256", "bytes", "created_task_id"})
        ...
        if "created_task_id" in (record.result or {}) and not _ID_RE.fullmatch(
            record.result["created_task_id"]
        ):
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation journal is malformed"
            )
        if (
            record.identity.operation == "task.create"
            and record.state == "applied"
            and "created_task_id" not in (record.result or {})
        ):
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation journal is malformed"
            )
        if record.identity.operation != "task.create" and "created_task_id" in (
            record.result or {}
        ):
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation journal is malformed"
        ...
        if record.failure is not None and (
            not isinstance(record.failure, dict)
            or set(record.failure) != {"code"}
            or record.failure["code"] not in {code.value for code in ErrorCode}
        ):
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation journal is malformed"
            )
        if record.state == "applied" and (
            record.result is None or record.failure is not None
        ):
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task mutation journal is malformed"
            )
        ...
        *,
        ...
        return self.execution.run(
            ...
            ExecutionProfile(
                ...
                max_stdout_bytes=max_stdout_bytes or MAX_TASK_OUTPUT_BYTES,
                max_stderr_bytes=MAX_TASK_STDERR_BYTES,
                max_combined_output_bytes=(max_stdout_bytes or MAX_TASK_OUTPUT_BYTES)
                + MAX_TASK_STDERR_BYTES,
                cwd=cwd,
    ...
    *,
    ...
    result = boundary.run(
        ...
        **(
            ...
            if max_stdout_bytes != MAX_TASK_OUTPUT_BYTES
            else {}
        ),
    )
    ...
    try:
        if json_lines:
            records = [
                json.loads(line) for line in result.stdout.decode().splitlines() if line
            ]
    ...
    for record in journal.records():
        if record.state == "dispatching":
            record = journal.failed(
                record,
                TaskError(ErrorCode.OWNER_UNAVAILABLE, "task outcome is unknown"),
            )
        elif record.state == "pending":
            record = journal.dispatching(record)
            try:
                result = _run_task_command(
                    boundary, authority, cwd, journal.intent(record), readonly=False
                )
                created_task_id = (
                    TaskService._created_task_id(result)
                    if record.identity.operation == "task.create"
                    else None
                )
            except TaskError as error:
                record = (
                    journal.pending(record, error)
                    if error.retryable
                    else journal.failed(record, error)
                )
            else:
                record = journal.applied(
                    record, result, created_task_id=created_task_id
                )
...
class TaskService:
    ...
    _locks_guard: threading.Lock = field(
        default_factory=threading.Lock, init=False, repr=False
    )
    _project_locks: dict[str, threading.Lock] = field(
    ...
    def execute(
        ...
        *,
        ...
        if not isinstance(arguments, dict):
            raise TaskError(
                ErrorCode.INVALID_ARGUMENT, "task arguments must be an object"
            )
        ...
        with self._lock_for(project.project_id), flock(self._lock_path(project)):
            if operation in _IDEMPOTENT_MUTATIONS:
                return self._submit_mutation(
                    project, operation, arguments, self._mutation_id(mutation_id)
                )
            return self._execute(project, operation, arguments, principal=principal)
    ...
    def _submit_mutation(
        ...
        task_id = (
            ...
            else self._task_id(arguments["task_id"])
        )
        idempotency_key = (
        ...
        identity = TaskMutationIdentity.create(
            project.project_id, operation, task_id, idempotency_key
        )
        arguments_sha256 = TaskMutationIdentity.digest(
            json.dumps(arguments, sort_keys=True, separators=(",", ":"))
        )
        authority = self._authority(project)
        journal = TaskMutationJournal(authority.root / TASK_MUTATION_JOURNAL_DIRECTORY)
        record = journal.load(identity, arguments_sha256) or journal.create(
            identity, arguments_sha256, command
        )
        if record.state == "dispatching":
            record = journal.failed(
                record,
            ...
            try:
                result = _run_task_command(
                    self.boundary, authority, project.root, command, readonly=False
                )
                created_task_id = (
                    self._created_task_id(result)
                    if operation == "task.create"
                    else None
                )
            except TaskError as error:
        ...
        if operation == "task.create":
            ...
            if record.state == "applied":
                ...
                response["task_ref"] = self._task_ref(
                    project.project_id, record.result["created_task_id"]
                )
        ...
        *,
        ...
        if operation == "task.list":
            ...
            result = self._single_task_result(
                self._run(
                    project,
                    ("show", self._task_id(arguments["task_id"])),
                    readonly=True,
                ),
                operation="task.get",
            )
            ...
            raise TaskError(
                ErrorCode.INVALID_ARGUMENT, f"unsupported task operation: {operation}"
            )
        return {
            "project_id": project.project_id,
            "operation": operation,
    ...
    def _mutation_command(
        ...
        if operation == "task.create":
            ...
            if "parent_task_id" in arguments:
                command.extend(
                    (
                        "--parent",
                        self._task_id(arguments["parent_task_id"], "parent_task_id"),
                    )
                )
            ...
            if dependencies:
                command.extend(
                    (
                        ...
                        ",".join(
                            f"{relation}:{task_id}"
                            for relation, task_id in dependencies
                        ),
                    )
                )
            return tuple(command)
        ...
        if operation == "task.update":
            self._require_exact(
                arguments, {"project_id", "task_id", "metadata"}, operation
            )
            metadata = arguments["metadata"]
            if not isinstance(metadata, dict) or not metadata or len(metadata) > 32:
                raise TaskError(
                    ErrorCode.INVALID_ARGUMENT,
                    "metadata must be a non-empty object with at most 32 entries",
                )
            ...
            for key, value in sorted(metadata.items()):
                command.extend(
                    (
                        "--set-metadata",
                        f"{self._string(key, 'metadata key', 256)}={self._string(value, 'metadata value', 4_000)}",
                    )
                )
            return tuple(command)
        if operation == "task.relate":
            self._require_exact(
                arguments, {"project_id", "task_id", "related_task_id"}, operation
            )
            return (
                "dep",
                "relate",
        ...
        if operation == "task.complete":
            self._require_allowed(
                arguments,
                {"project_id", "task_id", "merge_sha", "reason"},
                {"project_id", "task_id", "merge_sha"},
                operation,
            )
            ...
            if "reason" in arguments:
                command.extend(
                    ("--reason", self._string(arguments["reason"], "reason", 32_000))
                )
            return tuple(command)
        if operation == "task.release":
            self._require_allowed(
                arguments,
                {"project_id", "task_id", "reason", "if_assignee"},
                {"project_id", "task_id"},
                operation,
            )
            ...
            if "if_assignee" in arguments:
                command.extend(
                    (
                        "--if-assignee",
                        self._string(arguments["if_assignee"], "if_assignee", 256),
                    )
                )
            return tuple(command)
    ...
    def _list(
        self, project: ProjectAdapter, arguments: dict[str, Any], *, principal: str
    ) -> dict[str, Any]:
        ...
        if cursor is not None:
            ...
            if not isinstance(result, list):
                raise TaskError(
                    ...
                    "task backend returned an invalid list result",
                )
            rows = result
            if len(rows) > MAX_TASK_LIST_ROWS or any(
                not isinstance(row, dict) for row in rows
            ):
                raise TaskError(
                    ErrorCode.RESULT_INVALID, "task backend returned invalid list rows"
                )
        ...
        rows = snapshot["rows"]
        if offset > len(rows):
            raise TaskListCursorError(
                "task list cursor is beyond its snapshot", stale=True
            )
        ...
        if (
            ...
            raise TaskError(
                ErrorCode.RESOURCE_EXHAUSTED,
                "task list page exceeds the response bound",
            )
        return page
    ...
    def _list_query(
        ...
        if order is not None:
            if (
                ...
                or "field" not in order
            ):
                raise TaskError(
                    ErrorCode.INVALID_ARGUMENT,
                    "order must contain a supported field and optional reverse",
                )
            ...
            if not isinstance(field_name, str) or field_name not in {
                ...
                raise TaskError(
                    ErrorCode.INVALID_ARGUMENT, "order field is unsupported"
                )
        ...
        if "cursor" in arguments:
            ...
            if not isinstance(cursor, str) or not cursor:
                raise TaskError(
                    ErrorCode.INVALID_ARGUMENT, "task list cursor must be a string"
                )
            if len(cursor.encode()) > MAX_TASK_LIST_CURSOR_BYTES:
                raise TaskError(
                    ErrorCode.INVALID_ARGUMENT, "task list cursor exceeds its bound"
                )
        query = {
    ...
    def _task_list_cursor_key(self, project: ProjectAdapter) -> bytes:
        ...
        if len(key) != 32:
            raise TaskError(
                ErrorCode.OPERATION_FAILED, "task list cursor key is malformed"
            )
        return key
    ...
    def _encode_task_list_cursor(
        ...
        *,
        ...
        signature = hmac.new(
            self._task_list_cursor_key(project), encoded.encode(), hashlib.sha256
        ).digest()
        cursor = (
        ...
        if len(cursor.encode()) > MAX_TASK_LIST_CURSOR_BYTES:
            raise TaskError(
                ErrorCode.RESOURCE_EXHAUSTED, "task list cursor exceeds its bound"
            )
        return cursor
    ...
    def _decode_task_list_cursor(
        ...
        *,
        ...
        if (
            ...
            or not cursor
            or len(cursor.encode()) > MAX_TASK_LIST_CURSOR_BYTES
        ):
        ...
        expected_signature = (
            base64.urlsafe_b64encode(
                hmac.new(
                    self._task_list_cursor_key(project),
                    encoded.encode(),
                    hashlib.sha256,
                ).digest()
            )
        ...
        if cursor_principal != principal:
            raise TaskListCursorError(
                "task list cursor does not belong to this principal"
            )
    ...
    def _create_task_list_snapshot(
        self,
        project: ProjectAdapter,
        *,
        principal: str,
        ...
        if len(encoded) + 1 > MAX_TASK_LIST_SNAPSHOT_BYTES:
            raise TaskError(
                ErrorCode.RESOURCE_EXHAUSTED, "task list snapshot exceeds its bound"
            )
        root = self._authority(project).root / TASK_LIST_SNAPSHOT_DIRECTORY
        _ensure_durable_directory(root)
        snapshots = sorted(
            root.glob("*.json"), key=lambda path: path.stat().st_mtime_ns
        )
        for old in snapshots[: max(0, len(snapshots) - MAX_TASK_LIST_SNAPSHOTS + 1)]:
            old.unlink(missing_ok=True)
        self._write_bounded(
            self._task_list_snapshot_path(project, snapshot_id), encoded
        )
        return snapshot
    ...
    def _load_task_list_snapshot(
        ...
        *,
        ...
        snapshot = self._read_task_list_snapshot(
            self._task_list_snapshot_path(project, snapshot_id)
        )
        ...
        if snapshot["project_id"] != project.project_id:
            raise TaskListCursorError(
                "task list cursor does not belong to this project"
            )
        if snapshot["query_sha256"] != query_sha256:
            raise TaskListCursorError("task list cursor does not match this query")
        if snapshot["source_revision"] != source_revision:
            raise TaskListCursorError(
                "task list cursor source revision is stale", stale=True
            )
        return snapshot
    ...
    def _read_task_list_snapshot(self, path: Path) -> dict[str, Any]:
        try:
            ...
            raise TaskListCursorError(
                "task list cursor snapshot is unavailable", stale=True
            ) from error
        if (
            ...
            raise TaskListCursorError(
                "task list cursor snapshot is malformed", stale=True
            )
        if not isinstance(snapshot["snapshot_id"], str) or not re.fullmatch(
            r"[0-9a-f]{32}", snapshot["snapshot_id"]
        ):
            raise TaskListCursorError(
                "task list cursor snapshot is malformed", stale=True
            )
        if (
            not isinstance(snapshot["principal"], str)
            or not isinstance(snapshot["project_id"], str)
            or not isinstance(snapshot["query_sha256"], str)
            or not isinstance(snapshot["source_revision"], str)
            or not _SHA256_RE.fullmatch(snapshot["query_sha256"])
            or not _SHA256_RE.fullmatch(snapshot["source_revision"])
            or not isinstance(snapshot["rows"], list)
            or len(snapshot["rows"]) > MAX_TASK_LIST_ROWS
        ...
        if _canonical_digest(snapshot["rows"]) != snapshot["source_revision"]:
            raise TaskListCursorError(
                "task list cursor snapshot content is stale", stale=True
            )
        return snapshot

    def _task_list_snapshot_path(
        self, project: ProjectAdapter, snapshot_id: str
    ) -> Path:
        if not re.fullmatch(r"[0-9a-f]{32}", snapshot_id):
            raise TaskListCursorError("task list cursor snapshot is malformed")
        return (
            self._authority(project).root
            / TASK_LIST_SNAPSHOT_DIRECTORY
            / f"{snapshot_id}.json"
        )
    ...
    def _write_bounded(path: Path, encoded: bytes) -> None:
        if len(encoded) > MAX_TASK_LIST_SNAPSHOT_BYTES:
            raise TaskError(
                ErrorCode.RESOURCE_EXHAUSTED, "task list state exceeds its bound"
            )
    ...
    def _merge_sha(self, value: Any) -> str:
        ...
        if not _MERGE_SHA_RE.fullmatch(value):
            raise TaskError(
                ErrorCode.INVALID_ARGUMENT, "merge_sha must be a lowercase Git SHA"
            )
        return value
    ...
    def _authorize(principal: str, *, write: bool) -> None:
        if principal not in (_WRITE_PRINCIPALS if write else _READ_PRINCIPALS):
            raise TaskError(
                ...
                "task mutation requires an authorized principal"
                if write
                else "task read requires an authorized principal",
            )

    @staticmethod
    def _require_exact(
        arguments: dict[str, Any], expected: set[str], operation: str
    ) -> None:
    ...
    def _require_allowed(
        arguments: dict[str, Any], allowed: set[str], required: set[str], operation: str
    ) -> None:
        if set(arguments) - allowed or not required <= set(arguments):
            raise TaskError(
                ErrorCode.INVALID_ARGUMENT, f"{operation} received invalid arguments"
            )
    ...
    def _string(value: Any, name: str, maximum: int = 8_192) -> str:
        if not isinstance(value, str) or not value or len(value) > maximum:
            raise TaskError(
                ErrorCode.INVALID_ARGUMENT, f"{name} must be a non-empty bounded string"
            )
        return value
    ...
    def _labels(self, value: Any) -> tuple[str, ...]:
        if not isinstance(value, list) or len(value) > 32:
            raise TaskError(
                ErrorCode.INVALID_ARGUMENT, "labels must be a list of at most 32 labels"
            )
        if any(
    ...
    def _dependencies(self, value: Any) -> tuple[tuple[str, str], ...]:
        if not isinstance(value, list) or len(value) > 32:
            raise TaskError(
                ErrorCode.INVALID_ARGUMENT,
                "dependencies must be a list of at most 32 relations",
            )
        ...
        for dependency in value:
            ...
            if relation not in _DEPENDENCY_RELATIONS:
                raise TaskError(
                    ErrorCode.INVALID_ARGUMENT, "dependency relation is unsupported"
                )
            dependencies.append(
                (relation, self._task_id(dependency["task_id"], "dependency task_id"))
            )
    ...
    def _created_task_id(result: Any) -> str:
        if not isinstance(result, dict):
            raise TaskError(
                ErrorCode.RESULT_INVALID,
                "task backend returned an invalid task.create result",
            )
        value = result.get("id")
        if not isinstance(value, str) or not _ID_RE.fullmatch(value):
            raise TaskError(
                ErrorCode.RESULT_INVALID, "task backend omitted the created task"
            )
        return value
    ...
    def _single_task_result(result: Any, *, operation: str) -> dict[str, Any]:
        if (
            ...
            raise TaskError(
                ErrorCode.RESULT_INVALID,
                f"task backend returned an invalid {operation} result",
            )
        return result[0]
...
def task_reconcile_main(argv: list[str] | None = None) -> int:
    ...
    if (
        not _ID_RE.fullmatch(arguments.project_id)
        or not arguments.project_root.is_absolute()
        or not arguments.task_state_root.is_absolute()
    ):
    ...
    try:
        ...
        receipts = reconcile_task_mutations(
            ...
            boundary=BeadsCommandBoundary(),
        )
        sync = _run_task_command(
    ...
    print(
        json.dumps(
            {
                "state": "applied",
                "mutations": list(receipts),
                "sync_sha256": TaskMutationIdentity.digest(
