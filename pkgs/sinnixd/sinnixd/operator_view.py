...
STALE_RUNNING_SECONDS = {"harvest": 30 * 60, "verify_quick": 20 * 60, "verify_affected": 90 * 60, "verify_all": 120 * 60}
STALE_LANE_WAIT_SECONDS = 6 * 60 * 60
REACTOR_TICK_STALE_SECONDS = 5 * 60
GHOST_SECONDS = 24 * 60 * 60

ACTIVE_PHASES = frozenset({"queued", "capacity", "submitted", "waiting-dependencies", "running", "launching", "cancelling"})
...
def load_jobs(jobs_root: Path, project: str) -> list[dict[str, Any]]:
    ...
    for path in paths:
        try:
            ...
            continue
        if isinstance(value, Mapping) and (value.get("spec") or {}).get("project_id") == project:
...
def checks(
    ...
    *,
    ...
    if not reactor_active:
        problems.append("reactor is STOPPED: nothing advances until `systemctl --user start sinnixd-reactor`")
    elif last_dispatch is None or _seconds_since(last_dispatch, now) > REACTOR_TICK_STALE_SECONDS:
        problems.append(f"reactor has not dispatched for {_age(last_dispatch, now)} (lanes may all be waiting; see below)")
    corpus = status.get("master_corpus") or {}
    if not corpus:
        problems.append("no complete corpus run on record")
    elif not corpus.get("green"):
        problems.append(f"master corpus is RED: {corpus.get('red')} red / {corpus.get('passed')} passed at {corpus.get('head')} ({_age(corpus.get('finished_at'), now)} ago)")
    queued = [j for j in jobs if (j.get("state") or {}).get("phase") in {"queued", "capacity", "submitted", "waiting-dependencies"}]
    for job in sorted(queued, key=lambda j: str(j.get("created_at") or "")):
        ...
        if waited > STALE_QUEUED_SECONDS:
            ...
            problems.append(f"{_job_label(job)} {_job_workspace(job)} queued {_age(job.get('created_at'), now)} (blocked_by {','.join(map(str, blocked)) or '?'})")
            break
    for job in jobs:
        ...
        if limit and _seconds_since(job.get("created_at"), now) > limit:
            problems.append(f"{_job_label(job)} {_job_workspace(job)} running {_age(job.get('created_at'), now)}, past its usual {limit // 60}m")
...
def render_overview(
    ...
    *,
    ...
    lines: list[str] = []
    problems = checks(status, jobs, now=moment, reactor_active=reactor_active, last_dispatch=last_dispatch)
    lines.append(f"== {status.get('project_id')} at {moment.astimezone().strftime('%Y-%m-%d %H:%M')} "
    ...
    corpus = status.get("master_corpus") or {}
    if corpus:
        lines.append(f"== master corpus: {'GREEN' if corpus.get('green') else 'RED'} {corpus.get('red')} red / {corpus.get('passed')} passed, "
    ...
    ghosts = [j for j in active if _seconds_since(j.get("created_at"), moment) > GHOST_SECONDS]
    active = [j for j in active if j not in ghosts]
    lines.append(f"== jobs: {len(active)} active" + (f", {len(ghosts)} ghosts older than a day (cancel them: `agentctl job cancel <id>`)" if ghosts else ""))
    for job in ghosts[:3]:
        lines.append(f"  ghost {str(job.get('job_id'))[:8]} {_job_label(job):18} {_job_workspace(job):34} {(job.get('state') or {}).get('phase'):10} {_age(job.get('created_at'), moment):>6}")
    for job in active:
        state = job.get("state") or {}
        lines.append(f"  {str(job.get('job_id'))[:8]} {_job_label(job):18} {_job_workspace(job):34} {state.get('phase'):10} {_age(job.get('created_at'), moment):>6}")
    rows = list(status.get("lanes_next") or [])
    kinds = Counter(str((row.get("next") or {}).get("kind")) for row in rows)
    lines.append("== lanes: " + ", ".join(f"{count} {kind}" for kind, count in kinds.most_common()))
    order = ["park", "retry", "verify", "harvest", "publish", "integrate", "rebase", "review-fix", "await-sweep", "wait", "done", "idle"]
    for row in sorted(rows, key=lambda r: (order.index(str((r.get('next') or {}).get('kind'))) if str((r.get('next') or {}).get('kind')) in order else 99, str(r.get("workspace")))):
        nxt = row.get("next") or {}
        ...
        flags = f"{len(receipt.get('flags') or [])} flags" if receipt.get("flags") else ""
        lines.append(f"  {str(row.get('workspace')):34} {str(row.get('bead') or ''):18} {str(nxt.get('kind')):11} {str(nxt.get('reason') or '')[:48]:48} {pr} {flags}".rstrip())
    idle = [str(r.get("workspace")) for r in rows if (r.get("next") or {}).get("kind") in {"idle", "done"}]
    if idle:
    ...
    if errors:
        ...
        for item in errors[-5:]:
            lines.append(f"  {str(item.get('at') or item.get('recorded_at') or '')[:16]} {str(item.get('message') or item)[:120]}")
    return "\n".join(lines)


def _spool_events_for(spool: Path, job_ids: set[str], workspace: str) -> list[dict[str, Any]]:
    events: list[dict[str, Any]] = []
    try:
        with spool.open("r", encoding="utf-8", errors="replace") as handle:
            for line in handle:
                if workspace not in line and not any(job_id in line for job_id in job_ids):
                    continue
                try:
...
def render_lane_log(workspace: str, status: Mapping[str, Any], jobs: list[dict[str, Any]], spool: Path, *, now: datetime | None = None) -> str:
    ...
    lines = [f"== {workspace}"]
    row = next((r for r in status.get("lanes_next") or [] if r.get("workspace") == workspace), None)
    if row:
        nxt = row.get("next") or {}
        lines.append(f"bead {row.get('bead')} head {row.get('head')} pushed {row.get('pushed')} lane {row.get('lane')} holder {row.get('holder')}")
    ...
    for job in mine:
        ...
        entries.append((created, f"{_local(created)} {_job_label(job):18} {str(job.get('job_id'))[:8]} created"))
        if state.get("terminal"):
            entries.append((end, f"{_local(end)} {_job_label(job):18} {str(job.get('job_id'))[:8]} {phase} after {_age(created, _parse(end) or moment)}"))
        else:
            entries.append((created, f"{'':5} {'':18} {'':8} … {phase} for {_age(created, moment)}"))
    for event in _spool_events_for(spool, ids, workspace):
        ...
        detail = " ".join(f"{k}={str(v).replace(chr(10), ' ')}" for k, v in sorted(event.items()) if k not in {"kind", "emitted_at", "schema_version", "project", "workspace"} and not isinstance(v, (dict, list)))
