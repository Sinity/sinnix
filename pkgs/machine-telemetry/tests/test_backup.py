...
def seed_database(path: Path) -> None:
    with sqlite3.connect(path) as connection:
        ...
        connection.executemany("INSERT INTO samples(value) VALUES (?)", ((f"value-{index}",) for index in range(1000)))


def test_backup_completes_against_active_writer_and_decompresses_cleanly(tmp_path: Path) -> None:
    ...
    thread.start()
    try:
        result = subprocess.run([str(SCRIPT), str(source), str(output)], check=False, capture_output=True, text=True, timeout=30)
    finally:
        stop.set()
    ...
    subprocess.run(["zstd", "-q", "-d", "-f", str(output), "-o", str(restored)], check=True)
...
def test_failed_backup_removes_all_temporary_sidecars(tmp_path: Path) -> None:
    source = tmp_path
    output = tmp_path / "backup.sqlite.zst"
    result = subprocess.run([str(SCRIPT), str(source), str(output)], check=False, capture_output=True, text=True)
