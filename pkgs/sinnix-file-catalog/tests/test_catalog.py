...
def observation(path: Path, **extra: object) -> dict[str, object]:
    return {
        ...
        "tags": ["audio"],
        "inspections": [{"method": "manual", "scope": "metadata", "basis": "operator note"}],
        **extra,
    }


def import_rows(catalog: Path, rows: list[dict[str, object]]) -> subprocess.CompletedProcess[str]:
...
def test_import_is_idempotent_and_merges_evidence(tmp_path: Path) -> None:
    ...
    assert import_rows(catalog, [observation(payload, tags=["new"], inspections=[{"method": "tool", "scope": "content", "basis": "report"}])]).returncode == 0
...
def test_relocate_preserves_id_and_history_after_external_rename(tmp_path: Path) -> None:
...
def test_replacement_and_duplicate_path_are_refused_without_write(tmp_path: Path) -> None:
    payload = tmp_path / "recording.txt"
    payload.write_text("sample", encoding="utf-8")
    catalog = tmp_path / "catalog.json"
    assert import_rows(catalog, [observation(payload)]).returncode == 0
    ...
    assert import_rows(catalog, [observation(other), observation(other)]).returncode != 0
...
def test_dangling_relation_is_rejected_with_valid_inspection(tmp_path: Path) -> None:
    ...
    payload.write_text("sample", encoding="utf-8")
    catalog = tmp_path / "catalog.json"
    row = observation(payload, relations=[{"type": "derived", "target_id": str(uuid.uuid4()), "basis": "note"}])
    assert import_rows(catalog, [row]).returncode != 0
...
def test_description_history_and_unknown_fields_survive_updates(tmp_path: Path) -> None:
    ...
    payload.write_text("sample", encoding="utf-8")
    catalog = tmp_path / "catalog.json"
    assert import_rows(catalog, [observation(payload, identity={"attribution": "scanner"})]).returncode != 0
    ...
    assert import_rows(catalog, [observation(payload, title="Revised title", description="Revised description")]).returncode == 0
    asset = assets(catalog)[0]
...
def test_missing_identity_and_wrong_kind_are_rejected(tmp_path: Path) -> None:
    ...
    payload.write_text("sample", encoding="utf-8")
    catalog = tmp_path / "catalog.json"
    assert import_rows(catalog, [observation(payload, kind="collection")]).returncode != 0
    data = {
        "schema_version": 1,
...
def test_relocate_repairs_after_unrelated_payload_is_also_renamed(tmp_path: Path) -> None:
    first, second = tmp_path / "first", tmp_path / "second"
    first.write_text("one", encoding="utf-8")
    second.write_text("two", encoding="utf-8")
    catalog = tmp_path / "catalog.json"
    assert import_rows(catalog, [observation(first), observation(second)]).returncode == 0
