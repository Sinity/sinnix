...
from sinnix_audio_capture.segment import device_profile
...
def test_pw_record_argv_includes_target_when_given():
    # A sink is targeted by its own serial: its monitor ports live on the
    # sink node, and no `<sink>.monitor` node exists to name.
    argv = pw_record_argv("pw-record", device_profile(2), "10986")
    assert "--target" in argv
    assert argv[argv.index("--target") + 1] == "10986"
    assert argv[-1] == "-"


@pytest.mark.parametrize("profile", [device_profile(1), device_profile(2)])
