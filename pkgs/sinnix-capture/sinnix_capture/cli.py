...
def _cmd_write(args: argparse.Namespace) -> int:
    payload = json.loads(args.payload) if args.payload is not None else json.load(sys.stdin)
...
def main(argv: list[str] | None = None) -> int:
    ...
    write_p = sub.add_parser("write", help="Append one sinnix-capture-v1 record to a lane")
    ...
    write_p.add_argument("--raw-ref", default=None, help="Pointer to a raw artifact this record summarizes")
    ...
    query_p.add_argument("--since", type=float, default=0.0, help="Unix timestamp; records at or after this count toward records_since")
    query_p.add_argument("--lane", action="append", dest="lanes", help="Restrict to specific lane(s); default is every lane directory under capture-root")
