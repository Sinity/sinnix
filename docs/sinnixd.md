...
The optional `sinnixd-reactor` user service is the model-free owner of campaign mechanics. It consumes the append-only completion-event spool from a durable cursor, atomically maintains the versioned `campaign-board.json`, closes merged Beads only from a decision-time receipt, and marks successful Polylogue agent lanes `review_ready`. Its registered keeper reaction emits one actionable `keeper` event per pending action and exponentially backs off repeated emissions. It does not review, harvest, select, or dispatch work; those remain fresh-context harvester and strategist responsibilities.
...
On each first-observed terminal transition the daemon appends one JSON line — `{job_id, kind, project, phase, completed_at, checkout}` — to the event spool (`--event-spool`, default `/realm/state/agentctl/events.jsonl`). The spool is an append-only advisory watch point for supervisors (tail it instead of polling `job status`); it is never state authority, is written at most once per transition per daemon process, and rolls to `events.jsonl.old` past 64 MiB. A per-job `on_complete` hook is deliberately not implemented: the spool plus `wait --any` cover supervision without giving jobs ambient exec authority.

`agentctl agent list|status|wait|result` are supervision sugar over the same job routes filtered to `kind=attested-agent`; `agentctl agent launch` is an explicit alias for the bare dispatch form, and `job list --kind` filters any listing by job kind. Every attested-agent environment carries `BEADS_ACTOR=agent-<job id>` unless the project descriptor declares an explicit value, so task-authority writes from agents never default to the operator's identity. Terminal job records older than the 14-day retention window move to `jobs-archive/` at daemon start; archived records stay loadable by id while listings stop paying for unbounded history.
...
There are three host pools: `interactive`, `normal`, and `bulk`. The scheduler injects work into a pool only while its worker and estimated-memory budget allow it. It reads memory PSI before admitting a heavy normal or bulk job, leaves existing systemd jobs untouched during pressure, and continues to admit interactive work. A terminal systemd observation records `MemoryPeak` as the next estimate for that semantic operation. The learned estimate and successful-result cache are bounded durable state. If that state is malformed or unavailable after restart, it is discarded and the durable job records plus systemd remain the authority.
...
The Polylogue adapter can declare the harvest operation with `result = "json"` and executable `sinnixd-harvest`. An unauthorised invocation compiles a durable review receipt containing the merge-base diffstat, lane machine trailer, deterministic red flags, and a bounded full-diff reference, then returns with phase `review-required`; it never publishes. The coordinator supplies that `sinnix://harvest/<packet-id>` receipt to a second `--authorize` invocation. Authorization checks the receipt against the current checkout, serializes Git mutation, fetches and rebases, runs the quick gate with one pure baseline-displacement retry, pushes, creates and arms the pull request, and emits merge/bead-close events to the common AgentCTL spool. `HARVEST_OK`, `REBASE_CONFLICT`, and `GATE_RED` are typed JSON outcomes; unexpected dependency failures remain failed jobs. The descriptor should pass bounded file names for the PR title/body and bead close reason when those values contain spaces; the CLI also accepts a bare packet ID, which is safe for declared-operation parameter grammar.
