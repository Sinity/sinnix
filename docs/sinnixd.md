...
The daemon still does not own job queues, service leases, arbitrary shells beyond the typed operator contract, admission policy, Git history, hosted review state, or merge state. It owns durable submission and reconciliation evidence for typed task mutations, while Beads remains the task-state authority and GitHub remains authoritative for reviews and merges. Descriptor pool, cache, and exclusivity metadata remain descriptive until their existing authorities move behind an explicit shared contract. The gateway’s legacy controllers remain downstream and are unchanged here.
