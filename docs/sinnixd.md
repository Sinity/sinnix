...
The optional `sinnixd-reactor` user service is the model-free owner of campaign mechanics. It consumes the append-only completion-event spool from a durable cursor and performs receipt-bound mechanical reactions. `board` renders a 30-second cache from live AgentCTL jobs, Git/GitHub pull requests, and Beads; the cache is not campaign state and is never written by the reactor. The reactor does not review, harvest, select, or dispatch work; those remain fresh-context harvester and strategist responsibilities.
