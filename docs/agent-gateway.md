...
The existing `context`, `events`, `wait`, `get`, and `run` verbs carry the g2.10 surfaces. Context intents are `project.orientation`, `project.triage`, `bead.work`, `bead.review`, `job.review`, and `incident`. Each intent declares a bounded component plan. Components report available, unavailable, or stale state, owner source revisions, and one immutable context snapshot reference. Components are isolated, and the ephemeral gateway revision cache reuses a value only when the owner revision is identical. Snapshot retention is process-local and bounded. It is not a daemon or materialization store.

`events` normalizes existing gateway audit receipts, Sinnixd job observations, ops-reducer transitions, Beads owner revisions, and Git project revisions. Exact events retain their owner evidence. Revision observations are explicitly non-exact and do not claim an event that the owner did not provide. Continuations are authenticated opaque cursors bound to the principal and project scope. They contain only source positions and last observed owner revisions.

`wait` supports job terminal state, Beads status or revision, unit state, file hash, capture freshness, and receipt appearance. It polls synchronously within a maximum deadline. A timeout or cancellation returns current evidence and an opaque continuation. It never submits background work. Canonical resource templates are principal-filtered, and the pinned MCP SDK's in-process subscription bus publishes resource updates when observed owner revisions change. The five generated prompts are `orient-project`, `triage-beads`, `work-bead`, `review-job`, and `incident-orient`. They contain canonical refs and the same principal-filtered action catalog, but do not invoke actions or grant authority.
