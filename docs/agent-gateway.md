...
Local coordinators
    ...
    -> agentctl's launch and batch routes, in process
...
`pkgs/sinnix-mcp` is the shared protocol package for the gateway, future `agentctl` runtime, project adapters, and MCP owners. It owns the canonical `sinnix://` parser and templates, versioned request and response envelopes, bounded inline-or-opaque payload representation, typed errors, source-generation bindings, and a non-overlapping owner registry. Owner declarations name their authority and lifecycle (`read_only`, `daemon_owned`, `window_gated`, or `operator_confirmed`), so an MCP frontend cannot silently widen a domain owner’s write boundary. Archive-backed owners additionally bind returned facts and receipts to their source reference, generation, and root digest.
...
The job owner (`sinnix_agent_gateway/execution.py`) dispatches `job.start`, `job.agent.start`, `job.shell.start`, `job.get`, `job.wait`, `job.logs`, `job.result`, `job.list`, and `job.cancel` onto agentctl's launch and batch routes in process. A job's identity is its pueue task id; its state is pueue's phase, terminal flag and exit code. `run(action_name=operations.run)` queues a declared operation on the main checkout or a worktree. `run(action_name=agent.for_bead)` is `agentctl batch start` with that bead's dispatch group as the one worker. `run(action_name=shell.run)` queues one argv in the `interactive` pool inside the project's declared environment, with its working directory confined to the checkout; it accepts no root escalation, environment overlay, string-encoded command, or caller-selected unit. `context(intent=job.review)` returns the job, its typed result, its project and recent receipts. V2 `wait` is available to every job-reading principal, accepts only a canonical job ref and a 1-300 second bound, and rejects an owner response carrying a different job ID. `get` reads a bounded canonical job summary, log range, or result. V2 `operate` selects `jobs.cancel` for agent-control and operator, first verifies the requested phase, and returns the queue's `cancel_requested` answer. All responses pass through the common bounded V2 result envelope and retain typed owner failures.
