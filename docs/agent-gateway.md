...
Only the runner's scrubbed base environment plus an explicitly requested overlay reaches launched agents. Unrelated exported secrets are not inherited. Overlay values travel through a private mode-0600 JSON handoff and are not written to the manifest; only the overlay digest and interpreter/executable identity metadata are persisted. Job state and generated prompt, manifest, log, and artifact metadata are private to the user.
