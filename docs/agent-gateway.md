...
`agent-control` selects the `agents.run` and `jobs.cancel` V2 actions. `operator` selects `shell.run`, `machine.operate`, and `jobs.cancel`, while `files_write` and `tasks_write` remain outside the V2 contract. `tasks_read` invokes Beads with its physical read-only flag. `tasks_write` performs only supported structured native operations and uses `--append-notes`, never the history-replacing `--notes` form. `files_write` supports atomic replacement, append, mkdir, copy, move, and explicit regular-file removal. Copy and move refuse to replace a destination; moves create the destination before removing the source. A mutation can require the current SHA-256 so concurrent or stale requests fail rather than overwrite newer content. The typed job contract and owner boundary are described below.
