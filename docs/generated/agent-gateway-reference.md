...
Revision: `v3-typed-actions`. Catalog SHA-256: `daa7e61046c0ca022806a018d4552421504da37a3a86e57a491645a7a951383e`.
...
| `artifact`          | `artifacts`        | `sinnix://artifacts/{artifact_id}`                       | `artifacts.get`, `artifacts.list`, `artifacts.read`, `browser.screenshot`, `desktop.screenshot`, `documents.inspect`, `documents.render`                                                                                                                                                                                                                                                         |
...
Inspect an authorized image or PDF: dimensions, page count and selected page geometry.

Family: `query`. Owner: `documents`. Principals: `agent-control, observer, operator`. Typed failures: `conflict, deadline, idempotency_conflict, invalid_request, not_found, owner_failed, partial_completion, policy_denied, precondition_failed, response_bound, source_changed, stale_cursor, unavailable, unsupported_capability`.

Aliases: pdf pages, image dimensions.

Follow-up actions: `documents.render`.
...
Output: the response envelope's `data` field is `Inspection`; the full envelope schema is the `sinnix://gateway/v2/actions/documents.inspect` resource and `sinnix-agent-gateway catalog documents.inspect --schema`.
...
{
  "target": {
    "path": "/realm/documents/example.pdf"
  }
}
```
...
PDF pages are explicit and 1-based. Image crop coordinates follow EXIF orientation; only the first frame is rendered. Each view has page/source provenance and travels in ImageContent, independently of text read budgets. Original file bytes remain in a principal-scoped artifact. Decoder bounds: 80 million source image pixels, 16 million total output pixels, 2 GiB address space, 30 seconds and 3 MiB encoded images (4 MiB base64). Reduce pages or dimensions after response_bound. Client display depends on MCP image support.

Family: `query`. Owner: `documents`. Principals: `agent-control, observer, operator`. Typed failures: `conflict, deadline, idempotency_conflict, invalid_request, not_found, owner_failed, partial_completion, policy_denied, precondition_failed, response_bound, source_changed, stale_cursor, unavailable, unsupported_capability`.

Aliases: view photo, read PDF visually, image crop, thumbnail.

Follow-up actions: `documents.inspect`, `documents.render`, `artifacts.read`.
...
Output: the response envelope's `data` field is `RenderedDocument`; the full envelope schema is the `sinnix://gateway/v2/actions/documents.render` resource and `sinnix-agent-gateway catalog documents.render --schema`.
