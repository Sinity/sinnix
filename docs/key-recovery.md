...
> history payload). They now live at `/realm/state/secrets/sinnix/`, outside
...
> mitigation is to fold a copy of `/realm/state/secrets/sinnix/` into the same
...
# 3. Restore the encrypted secrets themselves to /realm/state/secrets/sinnix/
