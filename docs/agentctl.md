...
| `job clean <id> \| --all-terminal \| --daemon-era`                                                              | remove a terminal pueue task and disposable cancel marker; retain launch inputs and every execution attempt, output, result, outcome and evidence; archived reads use `--reference`; `--daemon-era` is a separate explicit legacy-state deletion                                                         |
